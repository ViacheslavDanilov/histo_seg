...
from .backend_api.services import events, projects, tasks
...
from .utilities.proxy_object import ProxyDictPostWrite, ProxyDictPreWrite
from .utilities.proxy_object import StubObject as _TaskStub
...
class Task(_Task):
    ...
    __task_id_reuse_time_window_in_hours = deferred_config(
        'development.task_reuse_time_window_in_hours',
        24.0,
        float,
    )
            ...
            if task_type not in Task.TaskTypes.__members__:
                raise ValueError(
                    "Task type '{}' not supported, options are: {}".format(
                        task_type,
                        Task.TaskTypes.__members__.keys(),
                    ),
                )
                ...
                # if this is the main process, create the task
                    ...
                    if cls._offline_mode:
                        ...
                        if output_uri is True:
                            output_uri = (
                                task.get_project_object().default_output_destination or True
                            )
            ...
            if auto_resource_monitoring and not is_sub_process_task_id:
                ...
                task._resource_monitor = resource_monitor_cls(
                    ...
                    report_mem_used_per_process=not config.get(
                        'development.worker.report_global_mem_used',
                        False,
                    ),
    ...
    def get_num_enqueued_tasks(cls, queue_name=None, queue_id=None):
        ...
        if result is None:
            raise ValueError(
                'Could not query the number of enqueued tasks in queue with ID {}'.format(queue_id),
            )
        return result
    ...
    def set_progress(self, progress):
        ...
        if not isinstance(progress, int) or progress < 0 or progress > 100:
            self.log.warning(
                "Can't set progress {} as it is not and int between 0 and 100".format(progress),
            )
            return
    ...
    def connect(self, mutable, name=None):
        ...
        raise Exception(
            'Unsupported mutable type %s: no connect function found' % type(mutable).__name__,
        )
        ...
        if remote_node_rank:
            ...
            for node_rank in range(1, master_conf.get('total_num_nodes', total_num_nodes)):
                ...
                node.set_system_tags(
                    node.get_system_tags() + [self._launch_multi_node_instance_tag],
                )
            ...
            for node_to_wait, rank in zip(
                ...
                self.log.info(
                    'Waiting for node with task ID {} and rank {}'.format(node_to_wait.id, rank),
                )
                ...
                self.log.info(
                    'Node with task ID {} and rank {} detected'.format(node_to_wait.id, rank),
                )
    ...
    def get_debug_samples(self, title, series, n_last_iterations=None):
        ...
        if isinstance(n_last_iterations, int) and n_last_iterations >= 0:
            samples = self._get_debug_samples(
                title=title,
                series=series,
                n_last_iterations=n_last_iterations,
            )
        else:
            raise TypeError(
    ...
    def import_offline_session(cls, session_folder_zip, previous_task_id=None, iteration_offset=0):
        ...
        try:
            ...
            raise ValueError(
                'Could not read Task object {}: Exception {}'.format(
                    session_folder / cls._offline_filename,
                    ex,
                ),
            )
        ...
        if previous_task_id:
            ...
            task_holding_reports = cls.import_task(
                export_data,
                target_task=task_holding_reports,
                update=True,
            )
        else:
    ...
    def _create_dev_task(
        ...
        if closed_old_task:
            logger.report_text(
                'ClearML Task: Closing old development task id={}'.format(default_task.get('id')),
            )
    ...
    def _connect_dictionary(self, dictionary, name=None):
        ...
        def _refresh_args_dict(task, config_proxy_dict):
            ...
            a_flat_dict = task._arguments.copy_to_dict(
                flatten_dictionary(config_proxy_dict),
                prefix=name,
            )
            # noinspection PyProtectedMember
            ...
            if (
                ...
                if self.__exit_hook:
                    ...
                    if (
                        ...
                        wait_for_uploads = (
                            self.__exit_hook.remote_user_aborted or self.__exit_hook.signal is None
                        )
                        if (
                            not self.__exit_hook.remote_user_aborted
