...
from ..backend_interface.util import exact_match_regex, get_or_create_project
from ..storage.util import hash_dict
from ..task import Task
from ..utilities.proxy_object import get_basic_type, verify_basic_type
...
class BaseJob(object):
    ...
    def get_metric(self, title, series):
        ...
        return tuple(
            response.response_data['tasks'][0]['last_metrics'][title][series][v] for v in values
        )
    ...
    def elapsed(self):
        ...
        if not self.task.data.started:
            ...
            if not self.task.data.started:
                return -1
        return (
            datetime.now(tz=self.task.data.started.tzinfo) - self.task.data.started
        ).total_seconds()
    ...
    def update_status_batch(cls, jobs):
        ...
        if not id_map or (
            time() - cls._last_batch_status_update_ts < 1 and not have_job_with_no_status
        ):
            return
        # noinspection PyProtectedMember
    ...
    def _create_task_hash(
        ...
        hyper_params = deepcopy(
            task.get_parameters() if params_override is None else params_override,
        )
        ...
        configs = (
            ...
            if configurations_override is None
            else configurations_override
        )
    ...
    def _set_task_cache_hash(cls, task, task_hash=None):
        ...
        if Session.check_min_api_version('2.13'):
            ...
            task._set_runtime_properties(
                runtime_properties={cls._job_hash_property: str(task_hash)},
            )
        else:
        ...
        if allow_caching:
            ...
            if task:
                if (
                    disable_clone_task
                    and self.task
                    and self.task.status == self.task.TaskStatusEnum.created
                ):
...
class LocalClearmlJob(ClearmlJob):
    ...
    def launch(self, queue_name=None):
        ...
        if Task.current_task() and not (Path(cwd) / local_filename).is_file():
            ...
            cwd = (
                os.path.abspath(os.path.join(os.getcwd(), os.sep.join(['..'] * levels)))
                if levels
                else os.getcwd()
            )
...
class RunningJob(BaseJob):
    ...
    def __init__(self, existing_task):  # noqa
        ...
        self.task = (
            ...
            else Task.get_task(task_id=existing_task)
        )
