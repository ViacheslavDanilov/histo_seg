...
from ..optimization import SearchStrategy
...
try:
    ...
    raise ImportError(
        "OptimizerOptuna requires 'optuna' package, it was not found\n"
...
class OptunaObjective(object):
    ...
    def objective(self, trial):
        ...
        current_job = self.optimizer.helper_create_job(
            self.base_task_id,
            ...
            if not is_pending:
                ...
                if iteration_value and iteration_value[1] is not None:
                    ...
                    if trial.should_prune() and (
                        not self.min_iteration_per_job
                        or iteration_value[0] >= self.min_iteration_per_job
                    ):
                        current_job.abort()
                    ...
                    if (
                        self.max_iteration_per_job
                        and iteration_value[0] >= self.max_iteration_per_job
                    ):
                        current_job.abort()
                        break
        ...
        print(
            'OptunaObjective result metric={}, iteration {}'.format(
                objective_metric,
                iteration_value,
            ),
        )
...
class OptimizerOptuna(SearchStrategy):
    def __init__(
        ...
        **optuna_kwargs,  # type: Any
        ...
        self._optuna_kwargs = dict(
            (k, v) for k, v in optuna_kwargs.items() if k in verified_optuna_kwargs
        )
    ...
    def _convert_hyper_parameters_to_optuna(self):
        ...
        for p in self._hyper_parameters:
            if isinstance(p, LogUniformParameterRange):
                ...
                raise ValueError(
                    'HyperParameter type {} not supported yet with OptimizerBOHB'.format(type(p)),
                )
