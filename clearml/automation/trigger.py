...
class BaseTrigger(BaseScheduleJob):
    ...
    def build_query(self, ref_time):
        return {
            ...
            self._update_field: '>{}'.format(
                ref_time.isoformat() if ref_time else self.last_update.isoformat(),
            ),
        }
    ...
    def verify(self):
        ...
        if self.tags and (
            ...
            or not all(isinstance(s, str) for s in self.tags)
        ):
            raise ValueError('Tags must be a list of strings: {}'.format(self.tags))
        if self.required_tags and (
            not isinstance(self.required_tags, (list, tuple))
            or not all(isinstance(s, str) for s in self.required_tags)
        ):
            raise ValueError(
                'Required tags must be a list of strings: {}'.format(self.required_tags),
            )
        ...
        query.update(
            {
                ...
                'task_types': list(
                    set(query.get('task_types', []) + [str(Task.TaskTypes.data_processing)]),
                ),
...
class TaskTrigger(BaseTrigger):
    ...
    def build_query(self, ref_time):
        ...
        if self.metrics and self.variant and self.threshold:
            ...
            filter_key = 'last_metrics.{}.{}.{}'.format(
                title,
                series,
                'max_value' if sign_max else 'min_value',
            )
    ...
    def verify(self):
        ...
        if self.on_status and not all(s in valid_status for s in self.on_status):
            raise ValueError(
                'Your on_status contains invalid status value: {}'.format(self.on_status),
            )
        valid_signs = ['min', 'minimum', 'max', 'maximum']
        if self.value_sign and self.value_sign not in valid_signs:
            raise ValueError(
                'Invalid value_sign `{}`, valid options are: {}'.format(
                    self.value_sign,
                    valid_signs,
                ),
            )
...
class TriggerScheduler(BaseScheduler):
    ...
    def add_model_trigger(
        ...
        trigger = ModelTrigger(
            ...
            add_tag=(add_tag if isinstance(add_tag, str) else (name or schedule_task_id))
            if add_tag
            else None,
    ...
    def _step(self):
        ...
        for trigger in self._model_triggers + self._dataset_triggers + self._task_triggers:
            ...
            try:
                ...
                self._log(
                    "Exception occurred while checking trigger '{}' state: {}".format(trigger, ex),
                )
    ...
    def __deserialize_section(self, section, trigger_class, current_triggers):
        ...
        try:
            return self.__deserialize_triggers(
                json.loads(json_str),
                trigger_class,
                current_triggers,
            )
        ...
        for executed_job in sorted(self._executed_triggers, key=lambda x: x.started, reverse=True):
            if not executed_job.finished:
                if executed_job.task_id:
                    ...
                    try:
                        a_thread = [
                            t for t in enumerate_threads() if t.ident == executed_job.thread_id
                        ]
    ...
    def __report_trigger_table(self, triggers, title):
        ...
        columns += [
            k for k in triggers[0].__dict__.keys() if k not in columns and not k.startswith('_')
        ]
        ...
        for j in triggers:
            ...
            row = [
                str(j_dict.get(c)).split('.', 1)[0]
                if isinstance(j_dict.get(c), datetime)
                else str(j_dict.get(c) or '')
                for c in columns
            ]
