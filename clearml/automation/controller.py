...
from .job import BaseJob, LocalClearmlJob, RunningJob
...
class PipelineController(object):
    ...
    _retries_callbacks = (
        {}
    )  # Node.name: Callable[[PipelineController, PipelineController.Node, int], bool]  # noqa
    _status_change_callbacks = (
        {}
    )  # Node.name: Callable[PipelineController, PipelineController.Node, str]
    ...
    class Node(object):
        ...
        explicit_docker_image = attrib(
            type=str,
            default=None,
        )  # The Docker image the node uses, specified at creation
        recursively_parse_parameters = attrib(
            type=bool,
            default=False,
        )  # if True, recursively parse parameters in
    ...
    def __init__(
        ...
        if not self._task:
            ...
            if self._pipeline_as_sub_project:
                ...
                get_or_create_project(
                    ...
                    system_tags=self._project_system_tags,
                )
        ...
        self._def_max_retry_on_failure = (
            retry_on_failure if isinstance(retry_on_failure, int) else 0
        )
    ...
    def set_default_execution_queue(self, default_execution_queue):
        ...
        """
        self._default_execution_queue = (
            str(default_execution_queue) if default_execution_queue else None
        )
    ...
    def add_step(
        ...
        if not base_task_factory and not base_task_id:
            if not base_task_project or not base_task_name:
                raise ValueError(
                    'Either base_task_id or base_task_project/base_task_name must be provided',
                )
            ...
            if not base_task:
                raise ValueError(
                    'Could not find base_task_project={} base_task_name={}'.format(
                        base_task_project,
                        base_task_name,
                    ),
                )
    ...
    def is_successful(self, fail_on_step_fail=True, fail_condition='all'):
        ...
        if fail_condition == 'all':
            ...
            raise UsageError(
                "fail_condition needs to be one of the following: 'all', 'failed', 'aborted'",
            )
    ...
    def get(
        ...
        if not pipeline_id:
            ...
            if pipelines:
                if not pipeline_version:
                    ...
                    for pipeline in pipelines:
                        ...
                        if not candidate_version or not Version.is_valid_version_string(
                            candidate_version,
                        ):
                            continue
            ...
            if not pipeline_id:
                error_msg = (
                    'Could not find dataset with pipeline_project={}, pipeline_name={}'.format(
                        pipeline_project,
                        pipeline_name,
                    )
                )
        ...
        try:
            pipeline_object._deserialize(
                pipeline_task._get_configuration_dict(cls._config_section),
                force=True,
            )
        except Exception:
            pass
    ...
    def _prepare_pipeline(
        ...
        if not self._verify():
            raise ValueError(
                'Failed verifying pipeline execution graph, '
    ...
    def _serialize_pipeline_task(self):
        ...
        if self._task and self._auto_connect_task:
            ...
            if (
                self._task.running_locally()
                or self._task.get_configuration_object(name=self._config_section) is None
            ):
                # noinspection PyProtectedMember
                ...
                mutable_dict = {
                    k: v for k, v in self._pipeline_args.items() if k not in connected_args
                }
                self._task.connect(
                    mutable_dict,
                    name=self._args_section,
                )
                new_pipeline_args.update(mutable_dict)
                ...
                if self._task._get_runtime_properties().get(self._runtime_property_hash):
                    ...
                    self._task._set_runtime_properties(
                        {
                            self._runtime_property_hash: '{}:{}'.format(
                                pipeline_hash,
                                self._version,
                            ),
                        },
            ...
            if not self._version:
                ...
                for previous_pipeline_task in previous_pipeline_tasks:
                    if previous_pipeline_task.runtime.get('version'):
                        self._version = str(
                            Version(
                                previous_pipeline_task.runtime.get('version'),
                            ).get_next_version(),
                        )
                        break
    ...
    def _deserialize(self, dag_dict, force=False):
        ...
        for name in list(self._nodes.keys()):
            if (
                not self._nodes[name].clone_task
                and name in dag_dict
                and not dag_dict[name].get('clone_task')
            ):
                for k in (
                    'queue',
                    'parents',
    ...
    def _has_stored_configuration(self):
        ...
        """
        if (
            self._auto_connect_task
            and self._task
            and not self._task.running_locally()
    ...
    def _verify_node(self, node):
        ...
        if not task:
            raise ValueError(
                "Node '{}', base_task_id={} is invalid".format(node.name, node.base_task_id),
            )
        ...
        def _verify_monitors(monitors, monitor_type, nested_pairs=False):
            ...
            if nested_pairs:
                ...
                conformed_monitors = [
                    pair if isinstance(pair[0], (list, tuple)) else (pair, pair)
                ...
                # verify the pair of pairs
                if not all(
                    isinstance(x[0], str) and isinstance(x[1], str) for x in conformed_monitors
                ):
                    raise ValueError(
                        '{} should be a list of tuples, found: {}'.format(monitor_type, monitors),
                    )
        ...
        # verify and fix monitoring sections:
        node.monitor_metrics = _verify_monitors(
            node.monitor_metrics,
            'monitor_metrics',
            nested_pairs=True,
        )
    ...
    def _add_function_step(
        ...
        for k, v in function_kwargs.items():
            ...
            if self._step_ref_pattern.match(str(v)):
                ...
                self._upload_pipeline_artifact(
                    artifact_name='{}.{}'.format(name, k),
                    artifact_object=v,
                )

        function_kwargs = {
            k: v for k, v in function_kwargs.items() if k not in function_input_artifacts
        }
        parameters = {
            '{}/{}'.format(CreateFromFunction.kwargs_section, k): v
            for k, v in function_kwargs.items()
        }
        ...
        if self._mock_execution:
            project_name = (
                project_name or self._get_target_project() or self._task.get_project_name()
            )
    ...
    def _relaunch_node(self, node):
        ...
        getLogger('clearml.automation.controller').warning(
            "Node '{}' failed. Retrying... (this is retry number {})".format(
                node.name,
                self._retries[node.name],
            ),
        )
    ...
    def _launch_node(self, node):
        ...
        for k, v in node.parameters.items():
            updated_hyper_parameters[k] = self._parse_step_ref(
                v,
                recursive=node.recursively_parse_parameters,
            )

        task_overrides = (
            self._parse_task_overrides(node.task_overrides) if node.task_overrides else None
        )
    ...
    def _call_retries_callback(self, node):
        ...
        try:
            ...
            getLogger('clearml.automation.controller').warning(
                "Failed calling the retry callback for node '{}'. Error is '{}'".format(
                    node.name,
                    e,
                ),
            )
    ...
    def _update_node_status(self, node):
        ...
        if (
            ...
            try:
                ...
                getLogger('clearml.automation.controller').warning(
                    "Failed calling the status change callback for node '{}'. Error is '{}'".format(
                        node.name,
                        e,
                    ),
                )
    ...
    def _update_progress(self):
        ...
        nodes = self._nodes.copy()
        job_progress = [
            (node.job.task.get_progress() or 0) if node.job else 0 for node in nodes.values()
        ]
        if len(job_progress):
        ...
        while self._stop_event:
            ...
            for j in self._running_nodes:
                ...
                if node.job.is_stopped(aborted_nonresponsive_as_running=True):
                    ...
                    if (
                        node_failed
                        and self._abort_running_steps_on_failure
            ...
            for node in list(self._nodes.values()):
                ...
                if node.job or node.executed or node.skip_job:
                    continue
                completed_parents = [
                    bool(p in self._nodes and self._nodes[p].executed) for p in node.parents or []
                ]
            ...
            node_launch_success = launch_thread_pool.map(
                self._launch_node,
                [self._nodes[name] for name in next_nodes],
            )
            for name, success in zip(next_nodes, node_launch_success):
                if success and not self._nodes[name].skip_job:
                    if (
                        self._nodes[name].job
                        and self._nodes[name].job.task_parameter_override is not None
                    ):
                        self._nodes[name].job.task_parameter_override.update(
                            self._nodes[name].parameters or {},
                        )
    ...
    def _verify_node_name(self, name):
        ...
        if name in self._reserved_pipeline_names:
            raise ValueError(
                'Node named \'{}\' is a reserved keyword, use a different name'.format(name),
            )
    ...
    def _monitor_node(self, node):
        ...
        if node.monitor_models:
            ...
            if target_models:
                ...
                models.output = [
                    a for a in models.output or [] if a.name not in keys
                ] + target_models
                # noinspection PyProtectedMember
        ...
        if max_value == page_size:
            ...
            try:
                ...
                last_task_name = self._task.query_tasks(
                    task_filter=dict(
                        task_ids=prev_pipelines_ids[:10],
                        project=[self._task.project],
                    ),
    ...
    def _get_pipeline_task(cls):
        ...
        if (
            str(task.task_type) == str(Task.TaskTypes.controller)
            and cls._tag in task.get_system_tags()
        ):
            return task
        ...
        if (
            str(parent.task_type) == str(Task.TaskTypes.controller)
            and cls._tag in parent.get_system_tags()
        ):
            return parent
        raise ValueError('Could not locate parent Pipeline Task')
    ...
    def __verify_step_reference(self, node, step_ref_string):
        ...
        if prev_step == self._pipeline_step_ref:
            if input_type not in self._pipeline_args:
                raise ValueError(
                    "Node '{}', parameter '{}', step name '{}' is invalid".format(
                        node.name,
                        v,
                        prev_step,
                    ),
        ...
        if input_type not in ('artifacts', 'parameters', 'models', 'id'):
            raise ValueError(
                "Node {}, parameter '{}', input type '{}' is invalid".format(
                    node.name,
                    v,
                    input_type,
                ),
    ...
    def __parse_step_reference(self, step_ref_string):
        ...
        if prev_step not in self._nodes or (
            ...
            raise ValueError(
                "Could not parse reference '{}', step '{}' could not be found".format(
                    step_ref_string,
                    prev_step,
                ),
            )
        ...
        if input_type not in (
            ...
            raise ValueError(
                "Could not parse reference '{}', type '{}' not valid".format(
                    step_ref_string,
                    input_type,
                ),
            )
        if input_type != 'id' and len(parts) < 3:
            raise ValueError(
                "Could not parse reference '{}', missing fields in '{}'".format(
                    step_ref_string,
                    parts,
                ),
            )
        ...
        task = (
            ...
            if self._nodes[prev_step].job
                ...
                task_id=self._nodes[prev_step].executed or self._nodes[prev_step].base_task_id,
            )
        )
        task.reload()
        if input_type == 'artifacts':
            ...
            for p in artifact_path:
                ...
                if isinstance(obj, dict):
                    ...
                    raise ValueError(
                        'Could not locate artifact {} on previous step {}'.format(
                            '.'.join(parts[1:]),
                            prev_step,
                        ),
                    )
            ...
            if param_name not in step_params:
                raise ValueError(
                    'Could not locate parameter {} on previous step {}'.format(
                        '.'.join(parts[1:]),
                        prev_step,
                    ),
                )
            ...
            if model_type not in ('input', 'output'):
                raise ValueError(
                    'Could not locate model {} on previous step {}'.format(
                        '.'.join(parts[1:]),
                        prev_step,
                    ),
                )
    ...
    def __create_task_link(cls, a_node, task_link_template):
        ...
        return '<a href="{}"> {} </a>'.format(
            task_link_template.format(project=project_id, task=task_id),
            task_id,
        )
    ...
    def _upload_pipeline_artifact(self, artifact_name, artifact_object):
        self._task.upload_artifact(
            ...
            extension_name=(
                '.pkl'
                if isinstance(artifact_object, dict) and not self._artifact_serialization_function
                else None
            ),
...
class PipelineDecorator(PipelineController):
    ...
    def _find_executed_node_leaves(self):
        ...
        executed_leaves = [
            name for name, n in list(self._nodes.items()) if n.executed and name not in all_parents
        ]
    ...
    def _adjust_task_hashing(self, task_hash):
        ...
        if task_hash.get('hyper_params'):
            ...
            for k, v in task_hash['hyper_params'].items():
                if k.startswith('{}/'.format(CreateFromFunction.input_artifact_section)) and str(
                    v,
                ).startswith('{}.'.format(self._task.id)):
    ...
    def _wait_for_node(cls, node):
        ...
        while True:
            ...
            if (
                ...
                or (
                    job_status == str(Task.TaskStatusEnum.failed)
                    and not cls._final_failure.get(node.name)
                )
    ...
    def component(
        ...
        *,
        ...
        def decorator_wrap(func):
            ...
            function_return = (
                return_values if isinstance(return_values, (tuple, list)) else [return_values]
            )
            ...
            if inspect_func.args:
                ...
                default_values = (
                    [None] * (len(inspect_func.args) - len(default_values))
                ) + default_values
            ...
            def wrapper(*args, **kwargs):
                if cls._debug_execute_step_function:
                    ...
                    def result_wrapper(a_func_return, return_index):
                        ...
                        return (
                            a_func_return if return_index is None else a_func_return[return_index]
                        )
                ...
                kwargs = {
                    k: deepcopy(v) for k, v in kwargs.items() if not isinstance(v, LazyEvalWrapper)
                }
                ...
                if not cls._singleton:
                    ...
                    target_queue = (
                        PipelineDecorator._default_execution_queue
                        or Task.current_task().data.execution.queue
                    )
                    if target_queue:
                ...
                def results_reference(return_name):
                    ...
                    if _node.job.is_failed() and not _node.continue_on_fail:
                        raise ValueError(
                            'Pipeline step "{}", Task ID={} failed'.format(
                                _node.name,
                                _node.job.task_id(),
                            ),
                        )
                ...
                def result_wrapper(return_name):
                    ...
                    cls._wait_for_node(_node)
                    if (
                        _node.job.is_failed() and not _node.continue_on_fail
                    ) or _node.job.is_aborted():
                        raise ValueError(
                            'Pipeline step "{}", Task ID={} failed'.format(
                                _node.name,
                                _node.job.task_id(),
                    ...
                    return task.get_parameters(cast=True)[
                        CreateFromFunction.return_section + '/' + return_name
                    ]
        ...
        *,  # noqa
        ...
        def decorator_wrap(func):
            def internal_decorator(*args, **kwargs):
                ...
                a_pipeline._clearml_job_class.register_hashing_callback(
                    a_pipeline._adjust_task_hashing,
                )
                ...
                if (
                    ...
                    and multi_instance_support
                    and cls._multi_pipeline_call_counter >= 0
                ):
                    # noinspection PyProtectedMember
                ...
                while waited:
                    ...
                    for node in list(a_pipeline._nodes.values()):
                        if (
                            node.executed
                            or not node.job
                            or node.job.is_stopped(aborted_nonresponsive_as_running=True)
                        ):
                            continue
                ...
                if return_value and pipeline_result is not None:
                    a_pipeline._upload_pipeline_artifact(
                        artifact_name=str(return_value),
    ...
    def _component_launch(cls, node_name, node, kwargs_artifacts, kwargs, tid):
        ...
        for k, v in kwargs.items():
            if v is None or isinstance(v, (float, int, bool, six.string_types)):
                ...
                cls._singleton._upload_pipeline_artifact(
                    artifact_name=artifact_name,
                    artifact_object=v,
                )
                _node.parameters[
                    '{}/{}'.format(CreateFromFunction.input_artifact_section, k)
        ...
        if PipelineDecorator._eager_execution_instance and _node.job:
            ...
            pipeline_tags = [
                t
                for t in Task.current_task().get_tags() or []
    ...
    def _add_pipeline_plots(cls, pipeline_task_id):
        ...
        if execution_flow:
            Task.current_task().get_logger().report_plotly(
                ...
                series='{} - {}'.format(
                    cls._report_plot_execution_flow['series'],
                    pipeline_task_id,
                ),
                iteration=0,
        ...
        if execution_details:
            Task.current_task().get_logger().report_plotly(
                ...
                series='{} - {}'.format(
                    cls._report_plot_execution_details['series'],
                    pipeline_task_id,
                ),
                iteration=0,
