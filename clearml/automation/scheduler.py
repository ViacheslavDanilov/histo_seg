...
class ScheduleJob(BaseScheduleJob):
    ...
    def verify(self):
        ...
        if self.weekdays and self.day not in (None, 0, 1):
            raise ValueError(
                '`weekdays` and `day` combination is not valid (day must be None,0 or 1)',
            )
        if self.weekdays and any(w not in self._weekdays_ind for w in self.weekdays):
            raise ValueError(
                '`weekdays` must be a list of strings, valid values are: {}'.format(
                    self._weekdays_ind,
                ),
            )
    ...
    def next(self):
        ...
        if self.weekdays:
            ...
            try:
                prev_weekday_ind = (
                    _weekdays.index(self._last_executed.weekday()) if self._last_executed else -1
                )
            except ValueError:
...
class BaseScheduler(object):
    ...
    def start(self):
        ...
        while True:
            ...
            if self._pooling_frequency_minutes:
                self._log(
                    'Sleeping until the next pool in {} minutes'.format(
    ...
    def _launch_job_task(self, job, task_parameters=None, add_tags=None):
        ...
        self._log(
            'Scheduling Job {}, Task {} on queue {}.'.format(
                job.name,
                task_job.task_id(),
                job.queue,
            ),
    ...
    def _launch_job_function(self, job, func_args=None):
        ...
        if job.single_instance and job.get_last_executed_task_id():
            ...
            try:
                a_thread = [
                    t for t in enumerate_threads() if t.ident == job.get_last_executed_task_id()
                ]
                if a_thread:
                    a_thread = a_thread[0]
            except Exception:
        ...
        self._log(
            "Scheduling Job '{}', Task '{}' on background thread".format(
                job.name,
                job.base_function,
            ),
        )
...
class TaskScheduler(BaseScheduler):
    ...
    def add_task(
        ...
        task_id = (
            schedule_task_id.id
            if isinstance(schedule_task_id, Task)
            else str(schedule_task_id or '')
        )
    ...
    def _step(self):
        ...
        # sort by key
        timeout_job_datetime = (
            min(self._timeout_jobs, key=self._timeout_jobs.get) if self._timeout_jobs else None
        )
        ...
        if timeout_job_datetime is not None:
            next_time_stamp = (
                min(next_time_stamp, timeout_job_datetime)
                if next_time_stamp
                else timeout_job_datetime
            )
    ...
    def _deserialize_state(self):
        ...
        if artifact_object is not None:
            ...
            if state_json_str is not None:
                ...
                self._timeout_jobs = {
                    datetime_from_isoformat(k): v for k, v in (state_dict.get('timeout_jobs') or {})
                }
                self._executed_jobs = [
                    ExecutedJob(**j) for j in state_dict.get('executed_jobs', [])
                ]
        ...
        for j in self._schedule_jobs:
            ...
            if row[1]:
                row[1] = '<a href="{}">{}</a>'.format(
                    task_link_template.format(project='*', task=row[1]),
                    row[1],
                )
            scheduler_table += [row]
    ...
    def _launch_job_task(self, job, task_parameters=None, add_tags=None):
        ...
        task_job = super(TaskScheduler, self)._launch_job_task(
            job,
            task_parameters=task_parameters,
            add_tags=add_tags,
        )
