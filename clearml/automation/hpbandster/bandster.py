        ...
        while not self._current_job.is_stopped():
            if is_pending and not self._current_job.is_pending():
                ...
                self.optimizer.budget.jobs.update(
                    ...
                    float(self.optimizer._min_iteration_per_job)
                    / self.optimizer._max_iteration_per_job,
                )
            ...
            if iteration_value:
                # update budget
                self.optimizer.budget.iterations.update(
                    self._current_job.task_id(),
                    iteration_value[0],
                )
...
class OptimizerBOHB(SearchStrategy, RandomSeed):
    ...
    def start(self):
        ...
        self._res = self._bohb.run(
            n_iterations=self.total_max_jobs,
