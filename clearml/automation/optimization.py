...
from ..backend_api.services import workers as workers_service
from ..backend_interface.util import datetime_from_isoformat, get_or_create_project
...
class SearchStrategy(object):
    ...
    def get_top_experiments_id_metrics_pair(self, top_k, all_metrics=False, only_completed=False):
        ...
        title, series = (
            self._objective_metric.get_objective_metric() if not all_metrics else (None, None)
        )
        return [
            (
                i,
    ...
    def helper_create_job(
        ...
        **kwargs,  # type: Any
        ...
        if parameter_override:
            param_str = [
                '{}={}'.format(k, parameter_override[k]) for k in sorted(parameter_override.keys())
            ]
            if self._naming_function:
                ...
                name = f'histology_segmentation_{today.strftime("%d%m_%H%M")}'
            comment = '\n'.join(param_str)
        else:
            name = None
            comment = None
        ...
        if self._objective_metric.get_objective(self._base_task_id) is None:
            logger.warning(
                'Could not find requested metric {} report on base task {}'.format(
                    self._objective_metric.get_objective_metric(),
                    self._base_task_id,
                ),
            )
...
class HyperParameterOptimizer(object):
    ...
    def __init__(
        ...
        **optimizer_kwargs,  # type: Any
        ...
        self._task = (
            auto_connect_task if isinstance(auto_connect_task, Task) else Task.current_task()
        )
        self._readonly_task = isinstance(auto_connect_task, Task) and str(
            self._task.status,
        ) not in ('created', 'in_progress')
        ...
        self.optimizer.set_job_default_parent(
            ...
            project_name=spawn_project or None,
        )
    ...
    def _connect_args(self, optimizer_class=None, hyper_param_configuration=None, **kwargs):
        ...
        configuration_dict = {
            'parameter_optimization_space': [c.to_dict() for c in hyper_param_configuration],
        }
        ...
        while self._thread is not None:
            ...
            if cur_task and not self._readonly_task:
                ...
                if (
                    ...
                    and self.optimizer.budget.compute_time.used
                ...
                self._report_completed_tasks_best_results(
                    set(completed_jobs.keys()),
                    task_logger,
                    title,
                    counter,
                )
    ...
    def _report_completed_status(
        ...
        if force or cur_completed_jobs != set(completed_jobs.keys()):
            ...
            job_ids_sorted_by_objective = sorted(
                ...
                reverse=bool(self.objective_metric.sign >= 0),
            )
            ...
            for i in range(1, len(table_values_with_links)):
                ...
                project_id = (
                    created_jobs_tasks[task_id].task.project
            ...
            if len(table_values) > 1:
                table_values_columns = [
                    [row[i] for row in table_values] for i in range(len(table_values[0]))
                ]
                table_values_columns = (
                    [
                        [table_values_columns[0][0]]
                        + [c[:6] + '...' for c in table_values_columns[0][1:]],
                    ]
    ...
    def _report_completed_tasks_best_results(self, completed_jobs, task_logger, title, counter):
        ...
        value_func, series_name = (
            (max, 'max') if self.objective_metric.get_objective_sign() > 0 else (min, 'min')
        )
    ...
    def _get_latest_completed_task_value(self, cur_completed_jobs, series_name):
        ...
        for j in cur_completed_jobs:
            ...
            if (
                not response.ok()
                or response.response_data['task'].get('status') != Task.TaskStatusEnum.completed
            ):
                continue
            completed_time = datetime_from_isoformat(
                response.response_data['task']['completed'].partition('+')[0],
            )
            completed_time = completed_time.timestamp()
            completed_values = self._get_last_value(response)
            obj_values.append(
                completed_values['max_value']
                if series_name == 'max'
                else completed_values['min_value'],
            )
    ...
    def _get_last_value(self, response):
        metrics, title, series, values = ClearmlJob.get_metric_req_params(
            self.objective_metric.title,
    ...
    def _auto_archive_low_performance_tasks(self, completed_jobs):
        ...
        for i, task_id in enumerate(job_ids_sorted_by_objective):
            ...
            if i < self._save_top_k_tasks_only and Task.archived_tag in system_tags:
                ...
                print(
                    'Archiving Task id={} (#{} objective={})'.format(
                        task_id,
                        i,
                        completed_jobs[task_id][0],
                    ),
    ...
    def __sort_jobs_by_objective(self, completed_jobs):
        if not completed_jobs:
            return []
        job_ids_sorted_by_objective = list(
            sorted(
                completed_jobs.keys(),
                key=lambda x: completed_jobs[x][0],
                reverse=bool(self.objective_metric.sign >= 0),
