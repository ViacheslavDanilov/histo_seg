...
from functools import WRAPPER_ASSIGNMENTS, reduce, wraps
...
from ....backend_api.session import Request as APIRequest
...
def make_action(service, request_cls):
    ...
    if action == 'get_by_id':
        ...
        def get(self, *args, **kwargs):
            return entity(
                self,
                self.session.send(request_cls(*args, **kwargs)).response,
            )
...
def make_service_class(module):
    ...
    properties.update(
        (f.__name__, f)
        for f in (make_action(module, value) for key, value in get_requests(module).items())
    )
...
class Version(Entity):
    ...
    def _get_default_kwargs(self):
        return dict(
            super(Version, self)._get_default_kwargs(),
