...
from ...backend_config.utils import apply_environment, apply_files
...
from .request import BatchRequest, Request  # noqa: F401
...
class Session(TokenManager):
    ...
    api_version = (
        '2.9'  # this default version should match the lowest api version we have under service
    )
    max_api_version = '2.9'
    feature_set = 'basic'
    ...
    def _connect(self):
        ...
        self._ssl_error_count_verbosity = self.config.get(
            'api.ssl_error_count_verbosity',
            self._ssl_error_count_verbosity,
        )

        self.__host = self.__init_host or self.get_api_server_host(config=self.config)
        if not self.__host:
            raise ValueError(
                'ClearML host was not set, check your configuration file or environment variable',
            )
        self.__host = self.__host.strip('/')
        self.__http_retries_config = (
            self.__init_http_retries_config
            or self.config.get(
                'api.http.retries',
                ConfigTree(),
        ...
        token_expiration_threshold_sec = self.config.get(
            'auth.token_expiration_threshold_sec',
            60,
        )
        ...
        if ENV_AUTH_TOKEN.get():
            ...
            self.__secret_key = self.__init_secret_key or ENV_SECRET_KEY.get(
                default=(
                    self.config.get('api.credentials.secret_key', None) or self.default_secret
                ),
            )
        ...
        if self._load_vaults():
            from clearml.config import ConfigSDKWrapper, ConfigWrapper
    ...
    def _update_default_api_method(self):
        if not ENV_API_DEFAULT_REQ_METHOD.get(default=None) and self.config.get(
    ...
    def _get_retry_codes(self):
        ...
        if retry_codes.difference(self._retry_codes):
            print(
                'Using extra HTTP retry codes {}'.format(
                    sorted(retry_codes.difference(self._retry_codes)),
                ),
            )
    ...
    def _read_vaults(self):
        ...
        def parse(vault):
            ...
            try:
                ...
                (self._logger or get_logger()).warning(
                    'Failed parsing vault {}: {}'.format(
                        vault.get('description', '<unknown>'),
                        e,
                    ),
                )
        ...
        try:
            ...
            res = self.send_request(
                'users',
                'get_vaults',
                params='enabled=true&types=config&types=config',
            )
            if res.ok:
    ...
    def _apply_config_sections(self, local_logger):
        ...
        if ENV_ENABLE_ENV_CONFIG_SECTION.get(default=default):
            try:
                ...
                local_logger().warning(
                    'Failed applying environment from configuration: {}'.format(ex),
                )
    ...
    def _send_request(
        ...
        url = (
            '{host}/v{version}/{service}.{action}' if version else '{host}/{service}.{action}'
        ).format(**locals())
        retry_counter = 0
        while True:
            ...
            if res.status_code == requests.codes.service_unavailable and self.config.get(
                'api.http.wait_on_maintenance_forever',
                True,
            ):
                (self._logger or get_logger()).warning(
    ...
    def send_request_batch(
        ...
        """
        if not all(
            isinstance(x, (list, tuple, type(None), types.GeneratorType)) for x in (data, json)
        ):
    ...
    def _do_refresh_token(self, old_token, exp=None):
        ...
        auth = (
            HTTPBasicAuth(self.access_key, self.secret_key)
    ...
    def __get_browser_token(webserver):
        ...
        if result_code != 200:
            raise ValueError(
                'Automatic authenticating failed, please login to {} and try again'.format(
                    webserver,
                ),
            )
    ...
    while not token:
        ...
        if not token:
            if not counter:
                ...
                print(
                    'Please login to {} , then press [Enter] to connect '.format(
                        clearml_app_server,
                    ),
                    end='',
                )
