...
if ENV_API_DEFAULT_REQ_METHOD.exists() and ENV_API_DEFAULT_REQ_METHOD.get().upper() not in (
    'GET',
    'POST',
    'PUT',
):
    ...
    def requests(self, value):
        ...
        if not self._allow_raw_requests:
            if any(isinstance(x, dict) for x in value):
                value = [
                    self._batched_request_cls(**x) if isinstance(x, dict) else x for x in value
                ]
