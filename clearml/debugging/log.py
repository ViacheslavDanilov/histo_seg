...
class LoggerRoot(object):
    ...
    def get_base_logger(cls, level=None, stream=sys.stdout, colored=False):
        ...
        if level is None and getenv('CLEARML_LOG_LEVEL'):
            ...
            if level is None:
                print(
                    'Invalid value in environment variable CLEARML_LOG_LEVEL: %s'
                    % getenv('CLEARML_LOG_LEVEL'),
                )
...
def add_options(parser):
    ...
    parser.add_argument(
        ...
        '-l',
        default=level,
        help='Log level (default is %s)' % level,
    )
...
class ClearmlTimedRotatingFileHandler(
