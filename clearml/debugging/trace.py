...
def _patch_module(
    ...
    if (
        prefix.startswith('clearml.backend_api.session.')
        and prefix != 'clearml.backend_api.session.'
    ):
...
def trace_trains(stream=None, level=1, exclude_prefixes=[], only_prefix=[]):
    ...
    stderr_write = (
        sys.stderr._original_write if hasattr(sys.stderr, '_original_write') else sys.stderr.write
    )
    if stream:
