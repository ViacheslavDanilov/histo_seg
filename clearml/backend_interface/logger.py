...
from ..config import DEBUG_SIMULATE_REMOTE_TASK, config, running_remotely
...
class PrintPatchLogger(object):
    ...
    def __init__(self, stream, logger=None, level=logging.INFO, load_config_defaults=True):
        if load_config_defaults and PrintPatchLogger.cr_flush_period is None:
            PrintPatchLogger.cr_flush_period = config.get(
                'development.worker.console_cr_flush_period',
                0,
            )
        ...
        if self._log and not PrintPatchLogger.recursion_protect_lock._is_owned():  # noqa
            try:
                ...
                if (
                    not do_flush
                    and do_cr
                    and PrintPatchLogger.cr_flush_period
                ...
                if (
                    not do_flush and (PrintPatchLogger.cr_flush_period or not do_cr)
                ) or not message:
                    return

                if PrintPatchLogger.cr_flush_period and self._cur_line:
                    self._cur_line = '\n'.join(
                        line.split('\r')[-1] for line in self._cur_line.split('\n')
                    )
