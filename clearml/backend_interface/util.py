...
from ..debugging.log import LoggerRoot, get_logger
...
def get_single_result(
    ...
    if len(results) > 1:
        if show_results:
            ...
            if show_results > 1:
                log.warning(
                    ...
                    ' (showing first {show_results} {entity}s follow)'.format(
                        num=len(results),
                        **locals(),
                    ),
                )
                ...
                log.warning(
                    '{num} {entity} found when searching for `{query}`'.format(
                        num=len(results),
                        **locals(),
                    ),
                )
        ...
        if sort_by_date:
            ...
            if hasattr(results[0], 'last_update'):
                results = sorted(
                    results,
                    key=lambda x: int(
                        (x.last_update - relative_time).total_seconds() if x.last_update else 0,
                    ),
                    reverse=True,
                )
        ...
        if raise_on_error:
            raise ValueError(
                'More than one {entity}s found when searching for ``{query}`'.format(**locals()),
            )
