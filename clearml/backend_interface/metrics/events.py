...
class UploadEvent(MetricsEventAdapter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if image_data is not None and (
            not hasattr(image_data, 'shape')
            and not isinstance(image_data, (six.StringIO, six.BytesIO))
        ):
    ...
    def _generate_file_name(self, force_pid_suffix=None):
        ...
        if not self._filename:
            ...
            cnt = (
                self._count
                if self.file_history_size < 1
                else (self._count % self.file_history_size)
            )
            self._filename += (
                '_{:05x}{:03d}'.format(force_pid_suffix, cnt)
                if force_pid_suffix
                else '_{:08d}'.format(cnt)
            )
        ...
        if isinstance(self._image_data, (six.StringIO, six.BytesIO)):
            ...
            if output is None:
                LoggerRoot.get_base_logger().warning(
                    'Skipping upload, could not find object file \'{}\''.format(
