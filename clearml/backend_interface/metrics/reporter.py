...
class Reporter(InterfaceBase, AbstractContextManager, SetupUploadMixin, AsyncManagerMixin):
    ...
    def is_constructed(self):
        # noinspection PyProtectedMember
        return self._report_service and (
            self._report_service.is_alive() or self._report_service._thread is True
        )
    ...
    def report_matplotlib(
        ...
        PatchedMatplotlib.report_figure(
            ...
            if not isinstance(force_save_as_image, str)
            else False,
        ...
        try:
            ...
            def default(obj):
                if isinstance(obj, (np.integer, np.int64)):
                    ...
                    return float(
                        round(obj, ndigits=round_digits) if round_digits is not None else obj,
                    )
    ...
    def report_image_plot_and_upload(
        ...
        _, url = ev.get_target_full_upload_uri(
            upload_uri or self._storage_uri,
            self._metrics.storage_key_prefix,
        )
