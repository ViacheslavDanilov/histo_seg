...
class Metrics(InterfaceBase):
    ...
    _file_upload_starvation_warning_sec = deferred_config(
        'network.metrics.file_upload_starvation_warning_sec',
        None,
    )
    ...
    def _do_write_events(self, events, storage_uri=None):
        ...
        if good_events:
            ...
            batched_requests = [
                api_events.AddRequest(event=ev, **additional_kwargs) for ev in _events if ev
            ]
            if batched_requests:
                if self._offline_mode:
                            ...
                            try:
                                ...
                                if plot.get('layout', {}).get('images'):
                                    for image in plot['layout']['images']:
                                        ...
                                        if (
                                            debug_sample not in uploaded_files
                                            and debug_sample.is_file()
                                        ):
                                            uploaded_files.add(debug_sample)
                                            StorageManager.upload_file(
                                                local_file=debug_sample.as_posix(),
