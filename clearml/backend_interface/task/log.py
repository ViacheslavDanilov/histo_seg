...
from ...utilities.process.mp import BackgroundMonitor, ForkEvent, ForkQueue, SafeEvent
...
class BackgroundLogService(BackgroundMonitor):
    ...
    def add_to_queue(self, record):
        ...
        if (
            self.is_subprocess_mode()
            and not self._fast_is_subprocess_alive()
            and not self.get_at_exit_state()
        ):  # HANGS IF RACE HOLDS!
    ...
    def _send_records(self, records):
        ...
        # noinspection PyBroadException
        try:
            record_events = [r for record in buffer for r in self._record_to_event(record)] + [
                self._last_event,
            ]
            self._last_event = None
            ...
            # noinspection PyProtectedMember
            TaskHandler._log_stderr(
                '{}\nWARNING: clearml.log - Failed logging task to backend ({:d} lines)'.format(
                    ex,
                    len(buffer),
                ),
            )
...
class TaskHandler(BufferingHandler):
    ...
    def shouldFlush(self, record):
        ...
        return (
            ...
            and self._background_log
            and self._background_log_size >= self.capacity
        )
    ...
    def close(self, wait=False):
        ...
        if _background_log:
            if not _background_log.is_subprocess_mode() or _background_log.is_alive():
                ...
                if wait and (
                    not _background_log.is_subprocess_mode()
                    or _background_log.is_subprocess_mode_and_parent_process()
                ):
                    # noinspection PyBroadException
                    try:
