...
from ...utilities.proxy_object import cast_basic_type, get_basic_type, verify_basic_type
...
class Task(IdObjectBase, AccessMixin, SetupUploadMixin):
    ...
    _report_subprocess_enabled = deferred_config(
        'development.report_use_subprocess',
        sys.platform == 'linux',
    )
    ...
    def _validate(self, check_output_dest_credentials=False):
        if not self._is_remote_main_task():
            self._storage_uri = (
                self.get_output_destination(raise_on_error=False, log_on_error=False) or None
            )
            return
    ...
    def _update_repository(self):
        ...
        try:
            ...
            result, script_requirements = ScriptInfo.get(
                ...
                else [sys.argv[0], self._calling_filename],
                log=self.log,
            ...
            if result.auxiliary_git_diff:
                diff_preview = (
                    '# git diff too large to handle, storing as artifact. git diff summary:\n'
                )
                diff_preview += '\n'.join(
                    line
                    for line in result.auxiliary_git_diff.split('\n')
                ...
                if self._force_use_pip_freeze:
                    if isinstance(self._force_use_pip_freeze, (str, Path)):
                        ...
                        requirements, conda_requirements = pip_freeze(
                            combine_conda_with_pip=config.get(
                                'development.detect_with_conda_freeze',
                                True,
                            ),
                        )
                    requirements = (
    ...
    def _auto_generate(self, project_name=None, task_name=None, task_type=TaskTypes.training):
        ...
        extra_properties = (
            {'system_tags': tags} if Session.check_min_api_version('2.3') else {'tags': tags}
        )
    ...
    def _set_storage_uri(self, value):
        ...
        self._edit(
            output_dest=self._storage_uri or ('' if Session.check_min_api_version('2.3') else None),
        )
    ...
    def _get_output_model(self, upload_required=True, model_id=None):
        ...
        return Model(
            ...
            or self.get_output_destination(
                raise_on_error=upload_required,
                log_on_error=upload_required,
            ),
    ...
    def _setup_reporter(self):
        ...
        self.__reporter = Reporter(
            metrics=self._get_metrics_manager(storage_uri=storage_uri),
            task=self,
        )
        return self.__reporter
    ...
    def _reload_field(self, field):
        ...
        with self._edit_lock:
            if self._offline_mode:
                ...
                res = self.send(
                    tasks.GetAllRequest(id=[self.id], only_fields=[field], search_hidden=True),
                )
        ...
        try:
            ...
            if delete_artifacts_and_models or callback:
                ...
                models_res = self.send(
                    models.GetAllRequest(task=[task.id], only_fields=['id', 'uri']),
                ).response.models
                models_res = [
                    m
                    for m in models_res
            ...
            event_uris.extend(
                [
                    x
                    for x in filter(None, self._get_image_plot_uris())
                    if not callback or callback('image_plot', x)
                ],
            )
                ...
                try:
                    ...
                    return next(
                        filter(
                            ...
                            (
                                image.get('source')
                                for image in plot.get('layout', {}).get('images', [])
                            ),
                        ),
                        None,
                    )
    ...
    def update_model_desc(self, new_model_desc_file=None):
        ...
        with self._edit_lock:
            ...
            model_desc_key = (
                list(execution.model_desc.keys())[0] if execution.model_desc else 'design'
            )
    ...
    def set_input_model(
        ...
        if model_id:
            ...
            if not model.ready:
                ...
                self.log.debug(
                    'Model %s [%s] is not published yet (not ready)' % (model_id, model.uri),
                )
            ...
            for section in hyperparams:
                for key, section_param in hyperparams[section].items():
                    parameters['{}/{}'.format(section, key)] = (
                        cast_basic_type(section_param.value, section_param.type)
                        if cast
                        else section_param.value
                    )
        else:
            for section in hyperparams:
                for key, section_param in hyperparams[section].items():
                    ...
                    if section_param.type == 'legacy' and section in (
                        self._legacy_parameters_section_name,
                    ):
    ...
    def _set_parameters(self, *args, **kwargs):
        ...
        # new parameters dict
        new_parameters = dict(
            itertools.chain.from_iterable(x.items() if isinstance(x, dict) else x for x in args),
        )
        ...
        with self._edit_lock:
            ...
            if not update and prefix:
                ...
                parameters = dict(
                    (k, v) for k, v in parameters.items() if not k.startswith(prefix + '/')
                )
            elif update:
                parameters = copy(self.get_parameters() or {})
            else:
                    ...
                    def create_description():
                        ...
                        if isinstance(v, Enum):
                            ...
                            created_description += 'Values:\n' + ',\n'.join(
                                [
                                    enum_key
                                    for enum_key in type(v).__dict__.keys()
                                    if not enum_key.startswith('_')
                                ],
                            )
    ...
    def remove_input_models(self, models_to_remove):
        ...
        with self._edit_lock:
            self.reload()
            self.data.models.input = [
                model for model in self.data.models.input if model.model not in ids_to_remove
            ]
    ...
    def _delete_artifacts(self, artifact_names, raise_on_errors=False, delete_from_storage=True):
        ...
        with self._edit_lock:
            ...
            if Session.check_min_api_version('2.13') and not self._offline_mode:
                ...
                execution = self.data.execution
                execution.artifacts = [
                    a for a in execution.artifacts or [] if a.key not in artifact_names
                ]
                self._edit(execution=execution)
        ...
        if uris:
            for i, (artifact, uri) in enumerate(zip(artifact_names, uris)):
                ...
                if uri and not self._delete_uri(uri) and raise_on_errors:
                    remaining_uris = {
                        name: uri for name, uri in zip(artifact_names[i + 1 :], uris[i + 1 :])
                    }
                    raise ArtifactUriDeleteError(
                        artifact=artifact,
                        uri=uri,
                        remaining_uris=remaining_uris,
                    )
    ...
    def set_project(self, project_id=None, project_name=None):
        ...
        if not project_id:
            ...
            if (
                not res
                or not res.response
                or not res.response.projects
                or len(res.response.projects) != 1
            ):
                return False
    ...
    def get_project_name(self):
        ...
        if self.project is None:
            return (
                ...
                if self._project_name and len(self._project_name) > 1
                else None
            )
    ...
    def get_project_object(self):
        ...
        if self.project is None:
            return (
                self._project_object[1]
                if self._project_object and len(self._project_object) > 1
                else None
            )

        if (
            self._project_object
            and self._project_object[1] is not None
            and self._project_object[0] == self.project
        ):
        ...
        while True:
            ...
            for event in response.get('events', []):
                ...
                if x_axis in ['timestamp', 'iter']:
                    ...
                    x_val = (
                        datetime.utcfromtimestamp(event['timestamp'] / 1000).isoformat(
    ...
    def get_projects(cls, **kwargs):
        ...
        while page == -1 or (
            res
            and res.response
            and res.response.projects
            and len(res.response.projects) == page_size
        ):
            page += 1
            res = cls._send(
            ...
            if res and res.response and res.response.projects:
                ret_projects.extend(
                    [projects.Project(**p.to_dict()) for p in res.response.projects],
                )
        return ret_projects
    ...
    def _edit(self, **kwargs):
        ...
        with self._edit_lock:
            ...
            status = (
                self._data.status if self._data and self._reload_skip_flag else self.data.status
            )
            if not kwargs.pop('force', False) and status not in (
                ...
                if kwargs and all(
                    k in ('name', 'project', 'comment', 'tags', 'system_tags', 'runtime')
    ...
    def _clone_task(
        ...
        if getattr(task, 'system_tags', None):
            extra['system_tags'] = [
                t for t in task.system_tags if t not in (cls._development_tag, cls.archived_tag)
            ]
        ...
        if task.script and task.script.requirements:
            cls._send(
                ...
                req=tasks.SetRequirementsRequest(
                    task=cloned_task_id,
                    requirements=task.script.requirements,
                ),
            )
        return cloned_task_id
    ...
    def _get_all_events(
        ...
        while returned_count < total_events and (
