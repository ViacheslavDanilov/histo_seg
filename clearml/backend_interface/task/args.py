...
class _Arguments(object):
    ...
    class _ProxyDictWrite(dict):
        ...
        def __init__(self, __arguments, __section_name, *args, **kwargs):
            ...
            self._section_name = (
                (__section_name.strip(_Arguments._prefix_sep) + _Arguments._prefix_sep)
                if __section_name
                else None
            )
    ...
    def copy_defaults_from_argparse(self, parser, args=None, namespace=None, parsed_args=None):
        (
            ...
            task_defaults_types,
        ) = self._get_defaults_from_argparse(
            parser,
            args=args,
            namespace=namespace,
    ...
    def copy_to_parser(self, parser, parsed_args):
            ...
            for parent_parser, current_action in matched_actions:
                if current_action and current_action.default == SUPPRESS and not v:
                    ...
                    try:
                        ...
                        if v:
                            if current_action.type:
                                v = [
                                    current_action.type(a) if str(a) != str(None) else None
                                    for a in v
                                ]
                    ...
                    if var_type == int:
                        ...
                        var_type = str
                    elif var_type not in (bool, int, float, str, dict, list, tuple) and str(
                    ...
                    if isfunction(current_action.type) and not v:  # noqa
                        ...
                        current_action.type == str
                        and current_action.default is None
                        and v in (None, '')
                    ):
                    ...
                    try:
                        if current_action.default is None and current_action.type != str and not v:
                            ...
                            not isfunction(current_action.type)
                            and current_action.default == current_action.type(v)
                        ) or (
                            isfunction(current_action.type)
                            and str(self.__cast_arg(current_action.default)) == str(v)
                        ):
        ...
        if Session.check_min_api_version('2.9') and not self._exclude_parser_args.get('*', None):
            ...
            try:
                ...
                missing_keys = [
                    k
                    for k in task_defaults.keys()
                    if k[len(self._prefix_args) :] not in task_arguments
                ]
                task_defaults = {k: v for k, v in task_defaults.items() if k in missing_keys}
                task_defaults_descriptions = {
                    k: v for k, v in task_defaults_descriptions.items() if k in missing_keys
                }
                task_defaults_types = {
                    k: v for k, v in task_defaults_types.items() if k in missing_keys
                }
                if task_defaults:
                    self._task.update_parameters(
                        task_defaults,
