...
class HyperParams(object):
    ...
    def edit_hyper_params(
        ...
        def make_item(value, name=None):
            if isinstance(value, tasks.ParamsItem):
                ...
                a_item = tasks.ParamsItem(
                    **{k: None if v is None else str(v) for k, v in value.items()},
                )
            elif (
                ...
                a_item = tasks.ParamsItem(
                    name=str(value[0]),
                    **{k: None if v is None else str(v) for k, v in value[1].items()},
                )
        ...
        if isinstance(iterables, dict):
            props.update(
                {name: make_item(name=name, value=value) for name, value in iterables.items()},
            )
        else:
            for i in iterables:
                item = make_item(i)
