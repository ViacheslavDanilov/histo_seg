...
class CreateAndPopulate(object):
    ...
    def __init__(
        ...
        if (
            repo
            and len(urlparse(repo).scheme) <= 1
            and not re.compile(self._VCS_SSH_REGEX).match(repo)
        ):
            folder = repo
            repo = None
        else:
        ...
        self.packages = (
            list(packages)
            if packages is not None and not isinstance(packages, bool)
    ...
    def create_task(self, dry_run=False):
        ...
        if self.folder or (self.script and Path(self.script).is_file() and not self.repo):
            self.folder = (
                os.path.expandvars(os.path.expanduser(self.folder)) if self.folder else None
            )
            self.script = (
                os.path.expandvars(os.path.expanduser(self.script)) if self.script else None
            )
            ...
            if not os.path.isfile(entry_point):
                raise ValueError(
                    "Script entrypoint file \'{}\' could not be found".format(entry_point),
                )
        ...
        if (
            ...
            raise ValueError(
                "Standalone script detected \'{}\', but no requirements provided".format(
                    self.script,
                ),
            )
        ...
        if repo_info:
            ...
            if self.cwd:
                ...
                cwd = (
                    ...
                    else (Path(repo_info.script['repo_root']) / self.cwd).as_posix()
                )
            ...
            if self.force_single_script_file and Path(self.script).is_file():
                ...
                if create_requirements:
                    task_state['script']['requirements'] = (
                        repo_info.script.get('requirements') or {}
                    )
        else:
            # standalone task
        ...
        if self.add_task_init_call:
            ...
            if self.repo or task_state.get('script', {}).get('repository'):
                ...
                # Add Task.init call
                task_init_patch += 'from clearml import Task\n' 'Task.init()\n\n'
        ...
        if self.verbose:
            ...
            if task_state['script'].get('requirements') and task_state['script'][
                'requirements'
            ].get('pip'):
                print(
                    'Requirements:{}{}'.format(
    ...
    def _set_output_uri(self, task):
        if self.output_uri:
            try:
                ...
                getLogger().warning(
                    'Could not verify permission for output_uri: "{}"'.format(self.output_uri),
                )
    ...
    def create_task_from_function(
        ...
        for f in helper_functions or []:
            helper_function_source, _ = CreateFromFunction.__extract_function_information(
                f,
                sanitize_function=_sanitize_helper_functions,
            )
