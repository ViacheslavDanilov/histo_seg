    ...
    def _get_command_output(self, path, name, command, commands=None, strip=True):
        ...
        try:
            ...
            if not name.endswith(self._fallback):
                fallback_command = attr.asdict(commands or self._get_commands()).get(
                    name + self._fallback,
                )
                if fallback_command:
                    try:
            ...
            self._get_logger().warning(
                "Can't get {} information for {} repo in {}".format(name, self.type_name, path),
            )
        ...
        if diff_from_remote and commands:
            ...
            info = attr.assoc(
                ...
                **{
                    ...
                    if command
                    and (
                        name.endswith(self._remote)
                        and not name[: -len(self._remote)].endswith(self._fallback)
                    )
                },
            )
...
class GitDetector(Detector):
    ...
    def get_info(self, path, include_diff=False, diff_from_remote=False):
        ...
        info = super(GitDetector, self).get_info(
            ...
            diff_from_remote=diff_from_remote,
        )
...
class EnvDetector(Detector):
    ...
    def _get_info(self, _, include_diff=False, diff_from_remote=None):
        ...
        modified = bool(
            diff or (status and [s for s in status.split('\n') if s.strip().startswith('M ')]),
        )
