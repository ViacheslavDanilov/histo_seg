        ...
        for r in conda_packages_json:
            ...
            if not r.get('channel') or r.get('channel') == 'pypi':
                ...
                pip_req_line = [
                    ...
                    if pip_l.split('==', 1)[0].strip() in name
                    or pip_l.split('@', 1)[0].strip() in name
                ]
                if pip_req_line and (
                    '@' not in pip_req_line[0]
                    or not pip_req_line[0].split('@', 1)[1].strip().startswith('file://')
                ):
                    req_lines.append(pip_req_line[0])
                    continue

                req_lines.append(
                    '{}=={}'.format(name[0], r['version'])
                    if r.get('version')
                    else '{}'.format(name[0]),
                )
                continue
            ...
            if name.startswith('_'):
                continue
            conda_lines.append(
                '{}=={}'.format(name, r['version']) if r.get('version') else '{}'.format(name),
            )
