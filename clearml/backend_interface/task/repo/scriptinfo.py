...
from ....config import VCS_DIFF, VCS_ENTRY_POINT, VCS_WORK_DIR, deferred_config
...
class _JupyterObserver(object):
    ...
    def _daemon(cls, jupyter_notebook_filename, notebook_name=None):
        ...
        try:
            ...
            from ....utilities.pigar.reqs import file_import_modules, get_installed_pkgs_detail
            ...
            try:
                ...
                if _script_exporter is None:
                    ...
                    if replace_ipython_display_pattern:
                        script_code = replace_ipython_display_pattern.sub(
                            r'\n\g<1>print(',
                            script_code,
                        )
...
class ScriptInfo(object):
    ...
    def _get_jupyter_notebook_filename(cls):
        ...
        if 'IPython' in sys.modules:
            ...
            try:
                ...
                local_ipynb_file = (
                    ip.__dict__.get('user_ns', {}).get('__vsc_ipynb_file__') if ip else None
                )
                if local_ipynb_file:
                    ...
                    cls._jupyter_install_post_store_hook(
                        local_ipynb_file.as_posix(),
                        log_history=False,
                    )
        ...
        try:
            ...
            current_kernel = (
                sys.argv[2].split(os.path.sep)[-1].replace('kernel-', '').replace('.json', '')
            )
            ...
            if is_google_colab:
                ...
                if not entry_point.exists():
                    ...
                    try:
                        ...
                        cls._get_logger().warning(
                            'Failed accessing jupyter notebook {}: {}'.format(notebook_path, ex),
                        )

                # if we failed to get something we can access, print an error
                if not entry_point.exists():
                    cls._get_logger().warning(
                        'Jupyter Notebook auto-logging failed, could not access: {}'.format(
                            entry_point,
                        ),
                    )
                ...
                cls._get_logger().warning(
                    'Failed accessing the jupyter server(s): {}'.format(jupyter_servers),
                )
    ...
    def _get_sagemaker_notebook(cls, current_kernel, timeout=30):
        ...
        try:
            ...
            with requests.Session() as s:
                ...
                jupyter_sessions = s.get(
                    unauthorized_url + '/jupyter/default/api/sessions',
                    timeout=timeout,
                ).json()
            ...
            cls._get_logger().warning(
                "Failed finding Notebook in SageMaker environment. Error is: '{}'".format(e),
            )
        return None, None
    ...
    def _get_script_info(
        ...
        if jupyter_filepath:
            ...
            cwd = cls._cwd()
            scripts_path = [
                Path(cls._absolute_path(os.path.normpath(f), cwd)) for f in filepaths if f
            ]
    ...
    def __legacy_jupyter_notebook_server_json_parsing(self):
        ...
        try:
            ...
            for f in glob(
                os.path.join(os.path.dirname(ipykernel.get_connection_file()), '??server-*.json'),
            ):
                # noinspection PyBroadException
                try:
    ...
    def detect_running_module(cls, script_dict):
        ...
        try:
            ...
            if cls.is_running_from_module():
                argvs = ''
                git_root = (
                    os.path.abspath(str(script_dict['repo_root']))
                    if script_dict['repo_root']
                    else None
                )
                for a in sys.argv[1:]:
