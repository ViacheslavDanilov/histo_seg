...
from ..backend_api.session.defs import ENV_ACCESS_KEY, ENV_OFFLINE_MODE, ENV_SECRET_KEY
        ...
        while True:
            ...
            try:
                ...
                if log and num_retries >= cls._num_retry_warning_display:
                    log.warning(
                        'Retrying, previous request failed %s: %s' % (str(type(req)), str(e)),
                    )
                ...
                if raise_on_errors:
                    raise ValidationError(
                        'Field %s contains illegal schema: %s' % ('.'.join(e.path), e.message),
                    )
