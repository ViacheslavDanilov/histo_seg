...
from clearml.backend_config.defs import LOCAL_CONFIG_FILE_OVERRIDE_VAR, LOCAL_CONFIG_FILES
...
def main():
    ...
    try:
        ...
        if parsed:
            ...
            web_server = get_parsed_field(
                parsed,
                ['web_server'],
            )  # TODO: if previous fails, this will fail too
...
def parse_known_host(parsed_host):
    if parsed_host.netloc.startswith('demoapp.'):
        ...
        print(
            'Assuming files and api ports are unchanged and use the same ('
            + parsed_host.scheme
            + ') protocol',
        )
...
def get_parsed_field(parsed_config, fields):
    ...
    ):  # fallback - try to parse the field like it was in web older version
        if len(fields) == 1:
