...
from .backend_interface.metrics import Metrics, Reporter
...
class BaseModel(object):
    ...
    def get_weights_package(
        ...
        for func in (zipfile.ZipFile, tarfile.open):
            ...
            raise ValueError(
                'cannot extract files from packaged model at %s',
                packed_file,
            )
    ...
    def report_scalar(self, title, series, value, iteration):
        ...
        """
        self._init_reporter()
        return self._reporter.report_scalar(
            title=title,
            series=series,
            value=float(value),
            iter=iteration,
    ...
    def _update_base_model(self, model_name=None, task_model_entry=None):
        ...
        model_name = (
            model_name
            or self._name
            or (self._floating_data.name if self._floating_data else None)
            or self._task.name
        )
        # noinspection PyBroadException
        try:
            task_model_entry = (
                task_model_entry or self._task_connect_name or Path(self._get_model_data().uri).stem
            )
        except Exception:
            pass
...
class OutputModel(BaseModel):
    ...
    def __init__(
        ...
        self._floating_data = create_dummy_model(
            ...
            comment='{} by task id: {}'.format(
                'Created' if not base_model_id else 'Overwritten',
                task.id,
            )
            + ('\n' + comment if comment else ''),
            framework=framework,
    ...
    def update_weights(
        ...
        if self._task.is_offline() and (
            weights_filename is None or not Path(weights_filename).is_dir()
        ):
        ...
        if weights_filename and not self.upload_storage_uri and not self._task.storage_uri:
            ...
            self._log.info(
                'No output storage destination defined, registering local model %s' % register_uri,
            )
    ...
    def _update_weights_offline(
        ...
        if weights_filename:
            weights_filename_offline = (
                self._task.get_offline_mode_folder()
                / self._offline_folder
                / Path(weights_filename).name
            ).as_posix()
