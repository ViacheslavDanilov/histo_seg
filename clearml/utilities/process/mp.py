...
class SingletonThreadPool(object):
    ...
    def is_active(cls):
        return (
            cls.__thread_pool
            and cls.__thread_pool_pid == os.getpid()
            and cls.__thread_pool.is_alive()
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self._q._writer._send_bytes = partial(
                SafeQueue._pipe_override_send_bytes,
                self._q._writer,
            )
        except Exception:
            pass
...
class BackgroundMonitor(object):
    ...
    def __start_subprocess_os_fork(cls, task_obj_id):
        ...
        if BackgroundMonitor._main_process == 0:
            ...
            BackgroundMonitor._main_process_proc_obj = psutil.Process(
                BackgroundMonitor._main_process,
            )
    ...
    def is_alive(self):
        ...
        return (
            self.is_subprocess_alive()
            and self._thread
            and self._start_ev.is_set()
            and not self._done_ev.is_set()
        )
    ...
    def _fast_is_subprocess_alive(cls):
        ...
        try:
            return (
                cls._main_process_proc_obj.is_running()
                and cls._main_process_proc_obj.status() != psutil.STATUS_ZOMBIE
            )
        except Exception:
            return False
    ...
    def is_subprocess_mode(self):
        return (
            self._subprocess is not False
            and bool(self._main_process)
            and self._task_id == self._main_process_task_id
        )
