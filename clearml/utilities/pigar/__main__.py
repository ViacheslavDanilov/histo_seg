...
from .reqs import is_base_module, is_std_or_local_lib, project_import_modules
...
class GenerateReqs(object):
    ...
    def extract_reqs(self, module_callback=None, entry_point_filename=None):
        ...
        for name in candidates:
            ...
            if name in self._installed_pkgs:
                ...
                for pkg, (pkg_name, version) in (
                    {'': pkginfo} if not isinstance(pkginfo, dict) else pkginfo
                ).items():
                    _name = pkg or name
        ...
        for name in self._local_mods:
            if name in modules and name not in reqs_module_name:
                ...
                if (
                    six.PY3
                    and os.path.commonpath([project_path])
                    == os.path.commonpath([project_path, mod_path])
                ) or (
                    six.PY2
                    and os.path.commonprefix([project_path])
                    == os.path.commonprefix([project_path, mod_path])
                ):
                    continue
    ...
    def _write_reqs(self, reqs):
        print(
            'Writing requirements to "{0}"'.format(
                self._save_path,
            ),
        )
