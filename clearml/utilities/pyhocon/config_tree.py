...
from .exceptions import ConfigException, ConfigMissingException, ConfigWrongTypeException
...
class ConfigTree(OrderedDict):
    ...
    def parse_key(string):
        ...
        return [
            token if contains_special_character(token) else token.strip('"') for token in tokens
        ]
    ...
    def with_fallback(self, config, resolve=True):
        ...
        if isinstance(config, ConfigTree):
            ...
            result = ConfigTree.merge_configs(
                ConfigFactory.parse_file(config, resolve=False),
                copy.deepcopy(self),
            )
    ...
    def as_plain_ordered_dict(self):
        ...
        return OrderedDict(
            (key.strip('"') if isinstance(key, (unicode, basestring)) else key, plain_value(value))
            for key, value in self.items()
        )
