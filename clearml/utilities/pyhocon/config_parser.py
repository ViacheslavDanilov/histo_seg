...
from .exceptions import ConfigException, ConfigMissingException, ConfigSubstitutionException
...
class ConfigParser(object):
    ...
    def parse(cls, content, basedir=None, resolve=True, unresolved_value=DEFAULT_SUBSTITUTION):
        ...
        with set_default_white_spaces():
            ...
            multiline_string = Regex('""".*?"*"""', re.DOTALL | re.UNICODE).setParseAction(
                parse_multi_string,
            )
            # single quoted line string
            quoted_string = Regex(r'"(?:[^"\\\n]|\\.)*"[ \t]*', re.UNICODE).setParseAction(
            ...
            value_expr = (
                period_expr | number_expr | true_expr | false_expr | null_expr | string_expr
            )
            ...
            inside_root_dict_expr = ConfigTreeParser(
                ZeroOrMore(
                    comment_eol | include_expr | assign_expr | eol_comma,
                ),
                root=True,
            )
    ...
    def postParse(self, instring, loc, token_list):
        ...
        for element in token_list:
            ...
            for tokens in expanded_tokens:
                # key, value1 (optional), ...
                key = (
                    tokens[0].strip() if isinstance(tokens[0], (unicode, basestring)) else tokens[0]
                )
                operator = '='
