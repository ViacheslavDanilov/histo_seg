...
def SVG_path(path, transform=None, simplify=False):
    ...
    vc_tuples = [
        (
            vertices if path_code != Path.CLOSEPOLY else [],
            PATH_DICT[path_code],
        )
...
def get_grid_style(axis):
    ...
    _gridOnMajor = (
        axis._gridOnMajor if hasattr(axis, '_gridOnMajor') else axis._major_tick_kw['gridOn']
    )
...
def get_axes_properties(ax):
    ...
    for axname in ['x', 'y']:
        ...
        if isinstance(axis.converter, matplotlib.dates.DateConverter):
            ...
            if pd is not None and isinstance(
                axis.converter,
                PeriodConverter,
            ):
