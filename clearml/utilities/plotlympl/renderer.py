...
class PlotlyRenderer(Renderer):
    ...
    def open_axes(self, ax, props):
        ...
        self.bar_containers = [
            c for c in ax.containers if c.__class__.__name__ == 'BarContainer'  # empty is OK
        ]
        self.current_bars = []

        # set defaults in axes
        xaxis = dict(
            anchor='y{0}'.format(self.axis_ct or ''),
            zeroline=False,
            ticks='inside',
        )
        yaxis = dict(
            anchor='x{0}'.format(self.axis_ct or ''),
            zeroline=False,
            ticks='inside',
        )
        zaxis = dict(
    ...
    def draw_bars(self, bars):
        ...
        for container in self.bar_containers:
            mpl_traces.append(
                [bar_props for bar_props in self.current_bars if bar_props['mplobj'] in container],
            )
