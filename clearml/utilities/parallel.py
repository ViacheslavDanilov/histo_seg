...
from six.moves.queue import Empty, PriorityQueue, Queue
        ...
        def zip(self, file_path, arcname=None):
            ...
            if self._verbose and self._task:
                self._task.get_logger().report_text(
                    'Compressing {}'.format(Path(file_path).as_posix()),
                )
        ...
        for file_path in file_paths:
            if not Path(file_path).is_file():
                LoggerRoot.get_base_logger().warning(
                    'Could not store dataset file {}. File skipped'.format(file_path),
                )
            else:
