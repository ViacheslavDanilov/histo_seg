...
def verify_basic_type(a_dict_list, basic_types=None):
    basic_types = (
        (float, int, bool, six.string_types)
        if not basic_types
    ...
    if isinstance(a_dict_list, (list, tuple)):
        ...
        return all(
            verify_basic_type(k, basic_types=basic_types) for k in a_dict_list.keys()
        ) and all(verify_basic_type(v, basic_types=basic_types) for v in a_dict_list.values())
...
def nested_from_flat_dictionary(a_dict, flat_dict, prefix='', sep='/'):
    ...
    for k, v in org_dict.items():
        ...
        if isinstance(v, (float, int, bool, six.string_types)):
            ...
            a_dict[k] = (
                nested_from_flat_dictionary(v, flat_dict, prefix=prefix + k + sep, sep=sep) or v
            )
        else:
            # this is a mixture of list and dict, or any other object,
            # leave it as is, we have nothing to do with it.
...
def walk_nested_dict_tuple_list(dict_list_tuple, callback):
    ...
    if type_dict_list_tuple == dict:
        ...
        for k, v in dict_list_tuple.items():
            ret[k] = (
                walk_nested_dict_tuple_list(v, callback=callback)
                if type(v) in nested
                else callback(v)
            )
        ...
        def make_method(name):
            def method(self, *args, **kwargs):
                ...
                if (
                    args
                    and len(args) == 1
                    and (type(args[0]) == LazyEvalWrapper or hasattr(type(args[0]), '_base_class_'))
                ):
                    try:
