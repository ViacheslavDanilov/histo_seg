...
class ResourceMonitor(BackgroundMonitor):
    ...
    def __init__(
        ...
        if not self._gpustat:
            ...
            try:
                active_gpus = os.environ.get('NVIDIA_VISIBLE_DEVICES', '') or os.environ.get(
    ...
    def daemon(self):
        ...
        while True:
            ...
            if fallback_to_sec_as_iterations is None:
                ...
                and seconds_since_started <= self.max_check_first_iteration
            ):
                if self._check_logger_reported():
                    fallback_to_sec_as_iterations = False
                    self._task.get_logger().report_text(
                        'ClearML Monitor: Reporting detected, '
    ...
    def _machine_stats(self):
        ...
        stats['memory_used_gb'] = (
            bytes_to_megabytes(
                self._get_process_used_memory() if self._process_info else virtual_memory.used,
            )
            / 1024
        )
        ...
        if self._gpustat:
            ...
            try:
                ...
                if self._gpustat_fail >= 3:
                    self._task.get_logger().report_text(
                        'ClearML Monitor: GPU monitoring failed getting GPU reading, '
    ...
    def _get_gpu_stats(self):
        ...
        if (
            ...
            gpu_stat = self._gpustat.new_query(per_process_stats=False)
            gpu_mem = (
                self._last_process_pool['gpu'][1] if self._last_process_pool.get('gpu') else None
            )
            ...
            if self._gpustat:
                ...
                if gpu_stat.gpus:
                    gpus = [
                        g
                        for i, g in enumerate(gpu_stat.gpus)
                        if not self._active_gpus or i in self._active_gpus
                    ]
                    specs.update(
                        gpu_count=int(len(gpus)),
                        gpu_type=', '.join(g.name for g in gpus),
                        gpu_memory=', '.join(
                            '{}GB'.format(round(g.memory_total / 1024.0)) for g in gpus
                        ),
