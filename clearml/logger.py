...
from .backend_interface.metrics.events import MetricsEventAdapter, UploadEvent
...
class Logger(object):
    ...
    **PLOTS**, and **DEBUG SAMPLES** sub-tabs. When you compare experiments, ``Logger`` output appears in the
    ...
    _tensorboard_single_series_per_graph = deferred_config(
        'metrics.tensorboard_single_series_per_graph',
        False,
    )
    ...
    def __init__(
        ...
        if self._task.is_main_task():
            StdStreamPatch.patch_std_streams(
                self,
                connect_stdout=connect_stdout,
                connect_stderr=connect_stderr,
            )
    ...
    def report_scalar(self, title, series, value, iteration):
        ...
        return self._task._reporter.report_scalar(
            title=title,
            series=series,
            value=float(value),
            iter=iteration,
    ...
    def report_matplotlib_figure(
        ...
        self._task._reporter.report_matplotlib(
            ...
            if report_interactive and not report_image
    ...
    def _report_file_and_upload(
        ...
        self._task._reporter.report_image_and_upload(
            ...
            if max_file_history is not None
            else self._default_max_sample_history,
            delete_after_upload=delete_after_upload,
        )
