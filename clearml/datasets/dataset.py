...
from .. import StorageManager, Task
...
from ..config import deferred_config, get_remote_task_id, running_remotely
from ..debugging.log import LoggerRoot
from ..storage.cache import CacheManager
from ..storage.helper import StorageHelper, cloud_driver_schemes
from ..storage.util import format_size, is_windows, md5text, sha256sum
...
class Dataset(object):
    ...
    __dataset_folder_template = CacheManager.set_context_folder_lookup(
        __cache_context,
        '{0}_archive_{1}',
    )
    ...
    __preview_tabular_table_count = deferred_config(
        'dataset.preview.tabular.table_count',
        10,
        transform=int,
    )
    __preview_tabular_row_count = deferred_config(
        'dataset.preview.tabular.row_count',
        10,
        transform=int,
    )
    __preview_media_image_count = deferred_config(
        'dataset.preview.media.image_count',
        10,
        transform=int,
    )
    __preview_media_video_count = deferred_config(
        'dataset.preview.media.video_count',
        10,
        transform=int,
    )
    __preview_media_audio_count = deferred_config(
        'dataset.preview.media.audio_count',
        10,
        transform=int,
    )
    __preview_media_html_count = deferred_config(
        'dataset.preview.media.html_count',
        10,
        transform=int,
    )
    __preview_media_json_count = deferred_config(
    ...
    def __init__(
        ...
        if task:
            ...
            if '/.datasets/' not in task.get_project_name() or '':
                ...
                if Dataset.is_offline() or bool(
                    Session.check_min_api_server_version(Dataset.__min_api_version),
                ):
                    get_or_create_project(
                        task.session,
    ...
    def version(self, version):
        ...
        if not Version.is_valid_version_string(version):
            LoggerRoot.get_base_logger().warning(
                "Setting non-semantic dataset version '{}'".format(version),
            )
        # noinspection PyProtectedMember
        ...
        if isinstance(dataset_path, str) or dataset_path is None:
            ...
            if len(dataset_path) != len(source_url):
                raise ValueError(
                    ...
                    ' (received {} paths for {} source urls))'.format(
                        len(dataset_path),
                        len(source_url),
                    ),
                )
    ...
    def remove_files(self, dataset_path=None, recursive=True, verbose=False):
        ...
        org_files = list(self._dataset_file_entries.keys()) + list(
            self._dataset_link_entries.keys(),
        )
    ...
    def upload(
        ...
        if not max_workers:
            max_workers = (
                1
                if self._task.output_uri
                and self._task.output_uri.startswith(tuple(cloud_driver_schemes))
            ...
            for zip_ in parallel_zipper.zip_iter(file_paths, arcnames=arcnames):
                ...
                for preview_entry in zip_.archive_preview[: Dataset.__preview_max_file_entries]:
                    ...
                    if (
                        ...
                        or len(truncated_preview) + total_preview_size
                        > Dataset.__preview_total_max_size
                    ):
    ...
    def finalize(self, verbose=False, raise_on_error=True, auto_upload=False):
        ...
        if self._dirty:
            if auto_upload:
                self._task.get_logger().report_text(
                    'Pending uploads, starting dataset upload to {}'.format(
                        self.get_default_storage(),
                    ),
                )
                self.upload()
                ...
                raise ValueError(
                    'Cannot finalize dataset, pending uploads. Call Dataset.upload(...)',
                )
            else:
                return False
    ...
    def set_metadata(self, metadata, metadata_name='metadata', ui_visible=True):
        ...
        if metadata_name.startswith(self.__data_entry_name_prefix):
            raise ValueError(
                "metadata_name can not start with '{}'".format(self.__data_entry_name_prefix),
            )
    ...
    def publish(self, raise_on_error=True):
        ...
        if not self.is_final():
            raise ValueError(
                'Cannot publish dataset, dataset in status {}.'.format(self._task.get_status()),
            )
    ...
    def get_mutable_local_copy(
        ...
        try:
            ...
            if not overwrite:
                if raise_on_error:
                    raise ValueError(
                        'Target folder {} already contains files'.format(target_folder.as_posix()),
                    )
                else:
                    return None
    ...
    def list_files(self, dataset_path=None, recursive=True, dataset_id=None):
        ...
        return sorted(
            [f for f in files if matches_any_wildcard(f, dataset_path, recursive=recursive)],
        )
    ...
    def list_removed_files(self, dataset_id=None):
        ...
        """
        datasets = (
            self._dependency_graph[self._id]
            if not dataset_id or dataset_id == self._id
            else [dataset_id]
        )
        unified_list = set()
    ...
    def list_added_files(self, dataset_id=None):
        ...
        added_list = [
            f
            for f in list(self._dataset_file_entries.keys())
            + list(self._dataset_link_entries.keys())
    ...
    def create(
        ...
        parent_datasets = [
            cls.get(dataset_id=p) if not isinstance(p, Dataset) else p
            for p in (parent_datasets or [])
        ]
        ...
        if dataset_name and not dataset_project and Task.current_task():
            LoggerRoot.get_base_logger().info(
                "Dataset project not provided, using Current Task's project",
            )
        ...
        if dataset_project is None and dataset_name is None and not use_current_task:
            LoggerRoot.get_base_logger().info(
                'New dataset project/name not provided, storing on Current Task',
            )
        ...
        if not dataset_project and not use_current_task:
            if not parent_datasets:
                raise ValueError(
                    'Missing dataset project name. Could not infer project name from parent dataset.',
                )
    ...
    def _raise_on_dataset_used(cls, dataset_id):
        ...
        if dependencies:
            raise ValueError(
                'Dataset id={} is used by datasets: {}'.format(
                    dataset_id,
                    [d.id for d in dependencies],
                ),
            )
    ...
    def _get_dataset_ids_respecting_params(
        ...
        if entire_dataset:
            ...
            hidden_dataset_project, _ = cls._build_hidden_project_name(
                dataset_project,
                dataset_name,
            )
            # noinspection PyProtectedMember
    ...
    def delete(
        ...
        if not any([dataset_id, dataset_project, dataset_name]):
            raise ValueError(
                "Dataset deletion criteria not met. Didn't provide id/name/project correctly.",
            )
        ...
        for dataset_id in dataset_ids:
            try:
                ...
                LoggerRoot.get_base_logger().warning(
                    'Could not get dataset with ID {}: {}'.format(dataset_id, str(e)),
                )
                continue
            # noinspection PyProtectedMember
                ...
                for external_file in dataset.link_entries:
                    if external_file.parent_dataset_id == dataset_id:
                        try:
                            ...
                            LoggerRoot.get_base_logger().warning(
                                "Failed deleting remote file '{}': {}".format(
                                    external_file.link,
                                    ex,
                                ),
                            )
    ...
    def rename(
        ...
        if not result:
            LoggerRoot.get_base_logger().warning(
                'Could not rename dataset with dataset_project={} dataset_name={}'.format(
                    dataset_project,
                    dataset_name,
                ),
            )
    ...
    def _move_to_project_aux(cls, task, new_project, dataset_name):
        ...
        """
        hidden_dataset_project_, parent_project = cls._build_hidden_project_name(
            new_project,
            dataset_name,
        )
        get_or_create_project(
    ...
    def move_to_project(
        ...
        if not bool(Session.check_min_api_server_version(cls.__min_api_version)):
            LoggerRoot.get_base_logger().warning(
                'Could not move dataset to another project because API version < {}'.format(
                    cls.__min_api_version,
                ),
            )
            return
        # noinspection PyBroadException
        ...
        for dataset_id in dataset_ids:
            ...
            if not dataset:
                LoggerRoot.get_base_logger().warning(
                    'Could not find dataset to move to another project',
                )
                continue
    ...
    def get(
        ...
        **kwargs,
        ...
        if not any([dataset_id, dataset_project, dataset_name, dataset_tags]):
            raise ValueError(
                "Dataset selection criteria not met. Didn't provide id/name/project/tags correctly.",
            )
        ...
        def get_instance(dataset_id_):
            ...
            if task.status == 'created':
                raise ValueError(
                    'Dataset id={} is in draft mode, delete and recreate it'.format(task.id),
                )
            force_download = (
                False if task.status in ('stopped', 'published', 'closed', 'completed') else True
            )
            ...
            if not dataset_id and not auto_create:
                raise ValueError(
                    'Could not find Dataset {} {}'.format(
                        ...
                        dataset_id
                        if dataset_id
                        else (dataset_project, dataset_name, dataset_version),
                    ),
                )
    ...
    def squash(
        ...
        datasets = (
            ...
            if dataset_ids
                ...
                cls.get(dataset_project=pair[0], dataset_name=pair[1])
                for pair in dataset_project_name_pairs
            ]
        )
        ...
        if len(datasets) == 1:
            ...
            for ds in datasets:
                ...
                parents = (
                    set(ds._get_parents())
                    if parents is None
                    else (parents & set(ds._get_parents()))
                )
            pool.close()
        ...
        return [
            {
                'name': d.name,
                'created': d.created,
                'project': cls._remove_hidden_part_from_dataset_project(
                    project_id_lookup[d.project],
                ),
                'id': d.id,
                'tags': d.tags,
    ...
    def _add_files(
        ...
        if path.is_file():
            ...
            file_entry = self._calc_file_hash(
                FileEntry(
                    ...
                    relative_path=(
                        Path(dataset_path or '.') / path.relative_to(local_base_folder)
                    ).as_posix(),
        ...
        self.update_changed_files(
            num_files_added=count - modified_count,
            num_files_modified=modified_count,
        )
    ...
    def _update_dependency_graph(self):
        ...
        used_dataset_versions = set(
            f.parent_dataset_id for f in self._dataset_file_entries.values()
        )
                ...
                try:
                    ...
                    if file.parent_dataset_id == self._id:
                        if file.relative_path in parent_file_entries:
                            ...
                            modified_files_size += (
                                file.size - parent_file_entries[file.relative_path].size
                            )
                        else:
                            added_files_count += 1
    ...
    def update_changed_files(
        self,
        num_files_added=None,
        num_files_modified=None,
        num_files_removed=None,
    ):
        """
    ...
    def _extract_dataset_archive(
        ...
        def _download_part(data_artifact_name):
            ...
            if not local_zip:
                raise ValueError(
                    'Could not download dataset id={} entry={}'.format(
                        self._id, data_artifact_name
                    ),
                )
            return local_zip
    ...
    def _get_data_artifact_names(self):
        ...
        data_artifact_entries = [
            ...
            if a
            and (
                a == self.__default_data_entry_name
                or str(a).startswith(self.__data_entry_name_prefix)
            )
        ]
    ...
    def _get_next_data_artifact_name(self, last_artifact_name=None):
        ...
        numbers = sorted(
            [int(a[prefix_len:]) for a in data_artifact_entries if a.startswith(prefix)],
        )
    ...
    def _merge_datasets(
        ...
        chunk_selection = (
            None if part is None else self._build_chunk_selection(part=part, num_parts=num_parts)
        )
        ...
        if target_base_folder and next(target_base_folder.iterdir(), None):
            if self._verify_dataset_folder(target_base_folder, part, chunk_selection):
                ...
                LoggerRoot.get_base_logger().info(
                    'Dataset needs refreshing, fetching all parent datasets',
                )
        ...
        if not self._verify_dataset_folder(target_base_folder, part, chunk_selection):
            LoggerRoot.get_base_logger().info(
                'Dataset parents need refreshing, re-fetching all parent datasets',
            )
            # we should delete the entire cache folder
            self._extract_parent_datasets(
                target_base_folder=target_base_folder,
                ...
                roots.extend(
                    list(
                        reversed(
                            [
                                p
                                for p in (self.get(dataset_id=r)._get_parents() or [])
                                if p not in roots
                            ],
                        ),
                    ),
                    ...
                    list(
                        reversed(
                            [
                                ...
                                if p not in roots
                                and (include_unused or (p in self._dependency_graph))
                            ],
                        ),
                    ),
                )
    ...
    def import_offline_session(cls, session_folder_zip, upload=True, finalize=False):
        ...
        dataset._dependency_graph = {
            (id if k.startswith('offline-') else k): [
                (id if sub_v.startswith('offline-') else sub_v) for sub_v in v
            ]
    ...
    def _build_dependency_chunk_lookup(self):
        ...
        chunks_lookup = map(
            lambda d: (
                d,
                (self if d == self.id else Dataset.get(dataset_id=d)).get_num_chunks(
    ...
    def _add_script_call(self, func_name, **kwargs):
        ...
        args = ', '.join(
            '\n    {}={}'.format(
                k,
                '\'' + str(v) + '\'' if isinstance(v, (str, Path, _Path)) else v,
            )
    ...
    def _report_dataset_genealogy(self):
        ...
        for node in nodes:
            count = 0
            size = 0
            for f in list(self._dataset_file_entries.values()) + list(
                self._dataset_link_entries.values(),
            ):
        ...
        for idx, node in enumerate(nodes):
            if node in self._dependency_graph:
                parents = [
                    visited.index(p) for p in self._dependency_graph[node] or [] if p in visited
                ]
            else:
                parents = [
                    visited.index(p)
                    for p in self.get(dataset_id=node)._get_parents() or []
                    if p in visited
                ]
            ...
            try:
                ...
                for node in dataset_version_node.values():
                    ...
                    for parent in node.get('parents', []):
                        ...
                        if (
                            parent_id not in dependency_graph_ex_copy
                            and parent_id not in dependency_graph_ex
                        ):
    ...
    def _report_dataset_preview(self):
            ...
            try:
                if file_extension_ == '.csv' and pd:
                    ...
                    if pyarrow:
                        ...
                        preview_rows = next(
                            pf.iter_batches(batch_size=self.__preview_tabular_row_count),
                        )
                        return pyarrow.Table.from_batches([preview_rows]).to_pandas()
                    elif fastparquet:
                        return (
                            fastparquet.ParquetFile(file_path_)
                            .head(self.__preview_tabular_row_count)
                            .to_pandas()
                        )
                elif (file_extension_ == '.npz' or file_extension_ == '.npy') and np:
                    return pd.DataFrame(
                        np.loadtxt(file_path_, max_rows=self.__preview_tabular_row_count),
                    )
            except Exception:
                pass
            return None
        ...
        for file in self._dataset_file_entries.values():
            ...
            if artifact is not None:
                ...
                try:
                    ...
                    if (
                        ...
                        and self._task.get_logger().get_default_upload_destination()
            ...
            if (
                ...
                and self.__preview_image_count < self.__preview_media_image_count
            ):
                self._task.get_logger().report_media('Images', file_name, local_path=file_path)
                self.__preview_image_count += 1
            elif (
                guessed_type.startswith('video')
                and self.__preview_video_count < self.__preview_media_video_count
            ):
                ...
                and self.__preview_audio_count < self.__preview_media_audio_count
            ):
                self._task.get_logger().report_media('Audio', file_name, local_path=file_path)
                self.__preview_audio_count += 1
            elif (
                guessed_type == 'text/html'
                and self.__preview_html_count < self.__preview_media_html_count
            ):
                ...
                and self.__preview_json_count < self.__preview_media_json_count
            ):
                self._task.get_logger().report_media(
                    'JSON',
                    file_name,
        ...
        for dataset_version_id in dependencies_by_order:
            ...
            def copy_file(file_entry):
                if file_entry.parent_dataset_id != dataset_version_id or (
                    ...
                    and self._get_chunk_idx_from_artifact_name(file_entry.artifact_name)
            ...
            if raise_on_error and any(errors):
                raise ValueError(
                    'Dataset merging failed: {}'.format([e for e in errors if e is not None]),
                )
        pool.close()
    ...
    def _add_external_files(
        ...
        try:
            if StorageManager.exists_file(source_url):
                ...
                if source_url[-1] != '/':
                    source_url = source_url + '/'
                remote_objects = StorageManager.list(
                    source_url,
                    with_metadata=True,
    ...
    def _build_chunk_selection(self, part, num_parts):
        ...
        ds_id_chunk_list = [
            (d, i) for d in dependencies for i in range(dependency_chunk_lookup.get(d, 1))
        ]

        # select the chunks for this part
        if part < leftover_chunks:
            indexes = ds_id_chunk_list[
                part * (avg_chunk_per_part + 1) : (part + 1) * (avg_chunk_per_part + 1)
            ]
        else:
            ds_id_chunk_list = ds_id_chunk_list[leftover_chunks * (avg_chunk_per_part + 1) :]
            indexes = ds_id_chunk_list[
                (part - leftover_chunks)
                * avg_chunk_per_part : (part - leftover_chunks + 1)
                * avg_chunk_per_part
            ]
                        ...
                        if (
                            ...
                            and (
                                (
                                    ...
                                    or not Version.is_valid_version_string(
    ...
    def _build_hidden_project_name(cls, dataset_project, dataset_name):
        ...
        if Dataset.is_offline() or bool(
            Session.check_min_api_server_version(cls.__min_api_version),
        ):
