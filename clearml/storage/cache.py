...
from ..config import deferred_config, get_cache_dir
...
class CacheManager(object):
    ...
    _default_cache_file_limit = deferred_config(
        'storage.cache.default_cache_manager_size',
        100,
    )
            ...
            def sort_max_access_time(x):
                ...
                try:
                    if x.is_dir():
                        ...
                        atime = (
                            max(atime, max(safe_time(s) for s in dir_files)) if dir_files else atime
                        )
                except Exception:
                    pass
                return atime
    ...
    def get_context_folder_lookup(cls, context):
        ...
        return cls._context_to_folder_lookup.get(
            str(context),
            cls._default_context_folder_template,
        )
