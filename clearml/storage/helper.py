...
from .callbacks import DownloadProgressReport, UploadProgressReport
...
class _HttpDriver(_Driver):
    ...
    class _Container(object):
        ...
        def _should_attach_auth_header(self):
            return any(
                (
                    self.name.rstrip('/') == host.rstrip('/')
                    or self.name.startswith(host.rstrip('/') + '/')
                )
    ...
    def upload_object_via_stream(
        ...
        **kwargs,
        ...
        if res.status_code != requests.codes.ok:
            raise ValueError(
                'Failed uploading object %s (%d): %s' % (object_name, res.status_code, res.text),
            )
    ...
    def _get_download_object(self, obj):
        ...
        if res.status_code != requests.codes.ok:
            raise ValueError(
                ...
                % (obj.object_name, res.status_code, res.reason),
            )
        return res
    ...
    def download_object(
        ...
        **_,
        ...
        if not overwrite_existing and p.is_file():
            self.get_logger().warning(
                'failed saving after download: overwrite=False and file exists (%s)' % str(p),
            )
            return
        length = 0
    ...
    def exists_file(self, container_name, object_name):
        ...
        try:
            ...
            return container.session.head(
                url,
                allow_redirects=True,
                headers=container.get_headers(url),
            ).ok
        except Exception:
            return False
...
class _Boto3Driver(_Driver):
    ...
    def _get_stream_download_pool(self):
        if self._stream_download_pool is None or self._stream_download_pool_pid != os.getpid():
            ...
            self._stream_download_pool = ThreadPoolExecutor(
                max_workers=int(self._stream_download_pool_connections),
            )
        ...
        try:
            ...
            container.bucket.upload_fileobj(
                ...
                Config=boto3.s3.transfer.TransferConfig(
                    ...
                    max_concurrency=int(self._max_multipart_concurrency)
    ...
    def _test_bucket_config(cls, conf, log, test_path='', raise_on_error=True, log_on_error=True):
        ...
        try:
            ...
            fullname = (
                furl(conf.bucket).add(path=test_path).add(path='%s-upload_test' % cls.__module__)
            )
            ...
            endpoint = (
                (('https://' if conf.secure else 'http://') + conf.host) if conf.host else None
            )
            boto_resource = boto_session.resource(
                's3',
            ...
            if region and (
                ...
                msg = 'incorrect region specified for bucket %s (detected region %s)' % (
                    conf.bucket,
                    region,
                )
            else:
...
class _GoogleCloudStorageDriver(_Driver):
    ...
    def list_container_objects(self, container, ex_prefix=None, **kwargs):
        ...
        try:
            ...
            return [
                blob for blob in container.bucket.list_blobs() if blob.name.startswith(ex_prefix)
            ]
...
class _AzureBlobServiceStorageDriver(_Driver):
        ...
        def _get_max_connections_dict(max_connections=None, key='max_connections'):
            ...
            try:
                max_connections = max_connections or int(
                    _AzureBlobServiceStorageDriver._max_connections,
                )
        ...
        try:
            ...
            container.create_blob_from_path(
                ...
                content_settings=ContentSettings(
                    content_type=get_file_mimetype(object_name or file_path),
                ),
                progress_callback=callback,
            )
            return True
    ...
    def get_object(self, container_name, object_name, *args, **kwargs):
        ...
        if not container:
            raise StorageError(
                'Container `{}` not found for object {}'.format(container_name, object_name),
            )
...
class _FileStorageDriver(_Driver):
    ...
    Object = namedtuple(
        'Object',
        ['name', 'size', 'extra', 'driver', 'container', 'hash', 'meta_data'],
    )
    ...
    def _make_object(self, container, object_name):
        ...
        full_path = os.path.realpath(
            os.path.join(self.base_path, container.name if container else '.', object_name),
        )
    ...
    def download_object(
        ...
        **_,
        ...
        if os.path.exists(file_path) and not overwrite_existing:
            raise ValueError(
                'File \"{}\" already exists, but overwrite_existing=False'.format(file_path),
            )
...
class StorageHelper(object):
    ...
    _gs_configurations = deferred_config(
        'google.storage',
        {},
        transform=GSBucketConfigurations.from_config,
    )
    _azure_configurations = deferred_config(
        'azure.storage',
        {},
        transform=AzureContainerConfigurations.from_config,
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._scheme == _AzureBlobServiceStorageDriver.scheme:
            ...
            new_url = normalize_local_path(
                url[len('file://') :] if url.startswith('file://') else url,
            )
    ...
    def get_azure_storage_uri_from_config(cls, bucket_config):
        return 'azure://{}.blob.core.windows.net/{}'.format(
            bucket_config.account_name,
            bucket_config.container_name,
        )
    ...
    def get_azure_configuration(cls, bucket_config):
        return cls._azure_configurations.get_config(
            bucket_config.account_name,
            bucket_config.container_name,
        )
    ...
    def _get_object_size_bytes(self, obj, silence_errors=False):
        ...
        try:
                ...
                if size is None and hasattr(obj, 'reload'):
                    ...
                    try:
                        ...
                        if not silence_errors:
                            self.log.warning(
                                "Failed obtaining object size on reload: {}('{}')".format(
            ...
            if not silence_errors:
                self.log.warning(
                    "Failed getting object size: {}('{}')".format(e.__class__.__name__, str(e)),
                )
        return size
    ...
    def upload(
        ...
        if cb and self.scheme in StorageHelper._quotable_uri_schemes:  # TODO: fix-driver-schema
            ...
            def callback(result):
                return a_cb(
                    quote_url(result_path, StorageHelper._quotable_uri_schemes)
                    if result
                    else result,
                )
    ...
    def list(self, prefix=None, with_metadata=False):
        ...
        if prefix:
            ...
            result = [
                obj.name if not with_metadata else self.get_object_metadata(obj) for obj in res
            ]
    ...
    def download_to_file(
        ...
        try:
            ...
            total_size_bytes = self.get_object_size_bytes(
                remote_path,
            ...
            try:
                ...
                if not path_local_path.is_file() or (
                    not skip_zero_size_check and path_local_path.stat().st_size <= 0
                ):
                    raise Exception(
                        'Failed renaming partial file, downloaded file exists and a 0-sized file',
                    )
    ...
    def conform_url(cls, folder_uri, base_url=None):
        ...
        if not folder_uri.startswith(_base_url):
            ...
            if _base_url == 'file://':
                ...
                raise ValueError(
                    'folder_uri: {} does not start with base url: {}'.format(folder_uri, _base_url),
                )
        ...
        if verbose:
            msg = 'Starting upload: {} => {}{}'.format(
                ...
                (
                    ...
                    if self._container.name.endswith('/')
                    else self._container.name + '/'
                )
