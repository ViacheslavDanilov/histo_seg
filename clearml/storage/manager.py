...
from .util import create_zip_directories, encode_string_to_filename, safe_extract
...
class StorageManager(object):
    ...
    def get_local_copy(
        ...
        if extract_archive and cached_file:
            ...
            cache_path_encoding = Path(cache.get_cache_folder()) / cache.get_hashed_url_file(
                remote_url,
            )
    ...
    def set_cache_file_limit(
        ...
        return CacheManager.get_cache_manager(
            ...
            cache_file_limit=cache_file_limit,
    ...
    def upload_folder(cls, local_folder, remote_url, match_wildcard=None):
        ...
        with ThreadPool() as pool:
            ...
            base_logger.error(
                'Failed uploading {}/{} files from {}'.format(
                    failed,
                    success + failed,
                    local_folder,
                ),
    ...
    def download_folder(
        ...
        if not results and not silence_errors:
            LoggerRoot.get_base_logger().warning(
                'Did not download any files matching {}'.format(remote_url),
            )
        return local_folder
    ...
    def list(cls, remote_url, return_full_path=False, with_metadata=False):
        ...
        try:
            ...
            LoggerRoot.get_base_logger().warning(
                "Can not list files for '{}' - {}".format(remote_url, ex),
            )
            return None
    ...
    def get_metadata(cls, remote_url, return_full_path=False):
        ...
        if return_full_path and not metadata['name'].startswith(base_url):
            metadata['name'] = (
                base_url + ('/' if not base_url.endswith('/') else '') + metadata['name']
            )
        return metadata
