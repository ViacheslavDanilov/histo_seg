...
def get_config_object_matcher(**patterns):
    ...
    starts_with = {
        k: v.rstrip('*') for k, v in patterns.items() if '*' not in v.rstrip('*') and '?' not in v
    }
...
def format_size(
    size_in_bytes,
    binary=False,
    use_nonbinary_notation=False,
    use_b_instead_of_bytes=False,
):
...
def parse_size(size, binary=False):
    ...
    if tokens and isinstance(tokens[0], (int, float)):
        ...
        disk_size_units = (
            (disk_size_units_b + disk_size_units_d)
            if binary
            else (disk_size_units_d + disk_size_units_b)
        )
        ...
        if normalized_unit:
            ...
            for k, low, high in disk_size_units:
                ...
                if normalized_unit in (low.lower(), high.lower()) or normalized_unit.startswith(
                    low.lower(),
                ):
                    return int(tokens[0] * k)
...
def get_common_path(list_of_files):
    ...
    for f in list_of_files:
        ...
        num_p = min(
            [
                i
                for i, (a, b) in enumerate(zip(common_path_parts[:num_p], f_parts[:num_p]))
                if a != b
            ]
            or [-1],
        )
