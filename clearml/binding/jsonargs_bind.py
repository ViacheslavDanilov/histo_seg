...
from ..utilities.proxy_object import flatten_dictionary, verify_basic_type
...
class PatchJsonArgParse(object):
    ...
    def patch(cls, task):
        ...
        if not cls.__patched:
            cls.__patched = True
            ArgumentParser.parse_args = _patched_call(
                ArgumentParser.parse_args,
                PatchJsonArgParse._parse_args,
            )
        ...
        for k, v in cls._args.items():
            ...
            if not verify_basic_type(v, basic_types=(float, int, bool, str, type(None))) and v:
                ...
                try:
                    ...
                    elif isinstance(v, Path) or (
                        isinstance(v, list) and all(isinstance(sub_v, Path) for sub_v in v)
                    ):
    ...
    def __delete_config_args(cls, parser, args, args_type, subcommand=None):
        ...
        for path in paths:
            args_to_delete = PatchJsonArgParse.__get_args_from_path(
                parser,
                path,
        ...
        if running_remotely():
            try:
                ...
                logging.getLogger(__file__).warning(
                    'Failed parsing jsonargparse arguments: {}'.format(e),
                )
    ...
    def _load_task_params(cls, parser=None):
        ...
        for key, section_param in cls.__remote_task_params[cls._section_name].items():
            if section_param.type == cls.namespace_type:
                params_dict['{}/{}'.format(cls._section_name, key)] = cls._get_namespace_from_json(
                    section_param.value,
                )
            elif section_param.type == cls.path_type:
                params_dict['{}/{}'.format(cls._section_name, key)] = cls._get_path_from_json(
                    section_param.value,
                )
            elif (
                not section_param.type or section_param.type == 'NoneType'
            ) and not section_param.value:
    ...
    def __get_args_from_path(parser, path, subcommand=None):
        with change_to_path_dir(path):
            parsed_cfg = parser.parse_string(
                path.get_content(),
                _skip_check=True,
                _fail_no_subcommand=False,
            )
            if subcommand:
                parsed_cfg = {
                    (
                        (subcommand + PatchJsonArgParse._commands_sep)
                        if k not in ['config', 'subcommand']
                        else ''
                    )
                    + k: v
