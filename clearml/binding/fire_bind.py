...
class PatchFire:
        ...
        if not cls.__patched:
            ...
            if running_remotely():
                ...
                fire.core._CallAndUpdateTrace = _patched_call(
                    fire.core._CallAndUpdateTrace,
                    PatchFire.__CallAndUpdateTrace,
                )
    ...
    def _update_task_args(cls):
        ...
        if cls.__current_command is None:
            ...
            parameters_types[
                cls._section_name + cls._args_sep + cls.__current_command
            ] = cls._command_type
            args.update(
                {
    ...
    def __CallAndUpdateTrace(  # noqa
        ...
        **kwargs,
        ...
        for k, v in PatchFire.__commands.items():
            ...
            if (
                inspect.ismethod(component)
                and inspect.ismethod(v)
                and v.__func__ == component.__func__
            ):
                PatchFire.__current_command = k
                break
            ...
            if frame_info.function == 'Fire':
                ...
                if inspect.getargvalues(frame_info.frame).locals['component'] is None:
                    ...
                    context = {
                        ...
                        and (not inspect.isclass(v) or v.__module__ != 'clearml.task')
                    }
                break
    ...
    def __get_all_groups_and_commands(component, context):
        ...
        component_trace_result = PatchFire.__safe_Fire(
            component,
            [],
            PatchFire.__default_args,
            context,
        ...
        while len(group_args) > 0:
            ...
            for command in current_commands:
                prefix = (
                    PatchFire._commands_sep.join(query_group) + PatchFire._commands_sep
    ...
    def __get_groups_and_commands_for_args(component, args_, parsed_flag_args, context, name=None):
        component_trace = PatchFire.__safe_Fire(
            component,
            args_,
            parsed_flag_args,
            context,
        ...
        groups, commands, _, _ = fire.helptext._GetActionsGroupedByKind(
            component_trace,
            verbose=True,
        )  # noqa
