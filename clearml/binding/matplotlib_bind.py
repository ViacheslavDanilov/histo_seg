...
class PatchedMatplotlib:
    ...
    def _get_global_image_counter_limit():
        ...
        if PatchedMatplotlib._global_image_counter_limit is None:
            ...
            PatchedMatplotlib._global_image_counter_limit = config.get(
                'metric.matplotlib_untitled_history_size',
                100,
            )
        ...
        try:
            ...
            for figure in figures:
                ...
                if figure.canvas.figure.stale or (
                    getattr(figure, '_trains_is_imshow', None) is not None
    ...
    def _report_figure(
        ...
        try:
            ...
            if report_as_debug_sample:
                ...
                image_format = (
                    force_save_as_image
                    if force_save_as_image and isinstance(force_save_as_image, str)
                    else 'jpeg'
                )
            elif force_save_as_image:
                # if this is an image, store as is.
                image_format = (
                    force_save_as_image if isinstance(force_save_as_image, str) else 'png'
                )
            else:
                image_format = 'svg'
                    ...
                    def our_mpl_to_plotly(fig):
                        ...
                        def process_tick_text(ticks, axes, val_index):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    _axis = next(
                                        x for x in axes if x in plotly_renderer.plotly_fig['layout']
                                    )
                                    plotly_renderer.plotly_fig['layout'][_axis].update(
                                        {
                                            'ticktext': [
                                                convert_latex_math_powers_to_html(t.get_text())
                                                for t in ticks
                                            ],
                                            'tickvals': [
                                                t.get_position()[val_index] for t in ticks
                                            ],
                                        },
                                    )
                                    if (
                                        plotly_renderer.plotly_fig['layout'][_axis].get('type')
                                        == 'linear'
                                    ):
                                        plotly_renderer.plotly_fig['layout'][_axis].pop(
                                            'type',
                                            None,
                                        )
                                except Exception:
                        ...
                        try:
                            ...
                            if (
                                ...
                                or len(plotly_renderer.plotly_fig.get('data', [{}])[0].get('z', []))
                                > 0
                            ):
                                process_tick_text(
                                ...
                                plotly_renderer.plotly_fig.setdefault('layout', {}).setdefault(
                                    'scene',
                                    {},
                                ).setdefault('camera', {}).setdefault('eye', {}).setdefault(
                                    'x',
                                    -1,
                                )
                                ...
                                plotly_renderer.plotly_fig.setdefault('layout', {}).setdefault(
                                    'scene',
                                    {},
                                ).setdefault('xaxis', {}).setdefault('autorange', 'reversed')

                                plotly_renderer.plotly_fig.setdefault('layout', {}).setdefault(
                                    'scene',
                                    {},
                                ).setdefault('yaxis', {}).setdefault('autorange', 'reversed')
                        except Exception:
                            pass
                        ...
                        if (
                            not plotly_renderer.plotly_fig.get('layout', {})
                            .get('xaxis', {})
                            .pop('custom_range', False)
                        ):
                            plotly_renderer.plotly_fig.get('layout', {}).get('xaxis', {}).pop(
                                'range',
                                None,
                            )
                        if (
                            ...
                            plotly_renderer.plotly_fig.get('layout', {}).get('yaxis', {}).pop(
                                'range',
                                None,
                            )
            ...
            if not title:
                ...
                if plot_title:
                    ...
                    title = 'untitled {:02d}'.format(
                        ...
                        % PatchedMatplotlib._get_global_image_counter_limit(),
                    )
                else:
    ...
    def _enforce_unique_title_per_iteration(title, last_iteration):
        ...
        if title in PatchedMatplotlib._last_iteration_plot_titles:
            ...
            title_last_iteration, title_counter = PatchedMatplotlib._last_iteration_plot_titles[
                title
            ]
