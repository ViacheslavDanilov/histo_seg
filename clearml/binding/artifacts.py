...
from ..storage.util import format_size, get_common_path, sha256sum
...
class Artifact(object):
    ...
    def __init__(self, artifact_api_object):
        ...
        self._metadata = (
            dict(artifact_api_object.display_data) if artifact_api_object.display_data else {}
        )
        self._preview = (
            artifact_api_object.type_data.preview if artifact_api_object.type_data else None
        )
        self._content_type = (
            artifact_api_object.type_data.content_type if artifact_api_object.type_data else None
        )
...
class Artifacts(object):
    ...
    def upload_artifact(
        ...
        if serialization_function:
            ...
            try:
                artifact_type_data.preview = (
                    preview or str(artifact_object.__repr__())[: self.max_preview_size_bytes]
                )
            except Exception:
                artifact_type_data.preview = ''
            ...
            if override_filename_ext_in_uri == '.json':
                ...
                try:
                    ...
                    if not auto_pickle:
                        raise
                    LoggerRoot.get_base_logger().warning(
                        "JSON serialization of artifact \'{}\' failed, reverting to pickle".format(
                            name,
                        ),
                    )
                    store_as_pickle = True
                ...
                try:
                    ...
                    if not auto_pickle:
                        raise
                    LoggerRoot.get_base_logger().warning(
                        "YAML serialization of artifact \'{}\' failed, reverting to pickle".format(
                            name,
                        ),
                    )
                    store_as_pickle = True
            ...
            if create_zip_file:
                ...
                if not artifact_object.is_file():
                    raise ValueError(
                        "Artifact file '{}' could not be found".format(artifact_object.as_posix()),
                    )

                override_filename_in_uri = artifact_object.parts[-1]
                artifact_type_data.preview = preview or '{} - {}\n'.format(
                    artifact_object.name,
                    format_size(artifact_object.stat().st_size),
                )
                ...
                with ZipFile(zip_file, 'w', allowZip64=True, compression=ZIP_DEFLATED) as zf:
                    for filename in sorted(list_files):
                        if filename.is_file():
                            ...
                            LoggerRoot.get_base_logger().warning(
                                "Failed zipping artifact file '{}', file not found!".format(
                                    filename.as_posix(),
                                ),
                            )
            except Exception as e:
            ...
            if preview:
                ...
                artifact_type_data.preview = (
                    '# full text too large to store, storing first {}kb\n{}'.format(
                        self.max_preview_size_bytes // 1024,
            ...
            try:
                ...
                self._delete_temp_file(local_filename)
                raise
        elif artifact_object is None or (
            isinstance(artifact_object, str) and artifact_object == ''
        ):
            artifact_type = ''
            store_as_pickle = True
        elif auto_pickle:
        ...
        if artifact_type_data.preview and len(artifact_type_data.preview) > (
            ...
            artifact_type_data.preview = (
                '# full preview too large to store, storing first {}kb\n{}'.format(
                    self.max_preview_size_bytes // 1024,
    ...
    def _upload_data_audit_artifacts(self, name):
        ...
        with self._task_edit_lock:
            ...
            artifact.display_data = (
                [(str(k), str(v)) for k, v in pd_metadata.items()] if pd_metadata else None
            )
    ...
    def _upload_local_file(
        ...
        upload_uri = (
            self._task.output_uri or self._task.get_logger().get_default_upload_destination()
        )
    ...
    def _push_temp_file(self, prefix=None, suffix=None):
        ...
        local_filename = Path(temp_folder) / (
            str(prefix).rstrip('.') + '.' + str(suffix).lstrip('.')
        )
    ...
    def _pop_temp_file(self, local_filename=None):
        ...
        try:
            ...
            raise ValueError(
                'Failed storing temp artifact into {}: error: {}'.format(local_filename, ex),
            )
