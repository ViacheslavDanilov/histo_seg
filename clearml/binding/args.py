...
class PatchArgumentParser:
    ...
    def parse_known_args(self, args=None, namespace=None):
        if PatchArgumentParser._recursion_guard:
            return (
                {}
                if not PatchArgumentParser._original_parse_args
                else PatchArgumentParser._original_parse_known_args(
                    self,
                    args=args,
                    namespace=namespace,
                )
        ...
        if current_task:
            ...
            try:
                ...
                current_task._connect_argparse(
                    ...
                    args=args,
                    namespace=namespace,
                    parsed_args=parsed_args_str[0]
                    if isinstance(parsed_args_str, tuple)
                    else parsed_args_str,
                )
            except Exception:
                pass
                        ...
                        def _get_sub_parsers_defaults(subparser, prev=[]):
                            ...
                            for actions in actions_grp:
                                _sub_parsers_defaults += [
                                    ...
                                    for v in actions
                                    if isinstance(v, _SubParsersAction)
    ...
    def _add_last_parsed_args(parser, parsed_args):
            ...
            if parsed_args_namespace and isinstance(parsed_args_namespace, Namespace):
                for k, v in parser._parsed_arg_string_lookup.items():  # noqa
                    if hasattr(parsed_args_namespace, k):
                        if isinstance(
                            getattr(parsed_args_namespace, k, None),
                            list,
        ...
        if (
            not PatchArgumentParser._last_parsed_args
            or parsed_args not in PatchArgumentParser._last_parsed_args
        ):
    ...
    def _add_last_arg_parser(a_argparser):
        PatchArgumentParser._last_arg_parser = (PatchArgumentParser._last_arg_parser or []) + [
            a_argparser,
        ]
