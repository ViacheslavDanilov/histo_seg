...
from ..config import TASK_LOG_ENVIRONMENT, config, running_remotely
...
class EnvironmentBind(object):
    ...
    def _bind_environment(cls):
        ...
        environ_log = str(TASK_LOG_ENVIRONMENT.get() or '').strip() or config.get(
            'development.log_os_environments',
            [],
        )
    ...
    def _patched_pool_worker(original_worker, *args, **kwargs):
        ...
        try:
            if len(args) >= 2 and hasattr(args[1], 'put'):
                ...
                kwargs['outqueue'] = SimpleQueueWrapper(
                    PatchOsFork._current_task,
                    kwargs['outqueue'],
                )
        except:  # noqa
            pass
