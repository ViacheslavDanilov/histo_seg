...
from ..config import DEV_TASK_NO_REUSE, get_remote_task_id, running_remotely
...
class PatchHydra(object):
        ...
        if PatchHydra.patch_hydra():
            ...
            if PatchHydra._last_untracked_state.get('_set_configuration'):
                ...
                PatchHydra._current_task._set_configuration(
                    **PatchHydra._last_untracked_state['_set_configuration'],
                )
    ...
    def _patched_hydra_run(self, config_name, task_function, overrides, *args, **kwargs):
        ...
        try:
            if running_remotely():
                ...
                PatchHydra._current_task.connect(
                    connected_config,
                    name=PatchHydra._parameter_section,
                )
                PatchHydra._allow_omegaconf_edit = connected_config.pop(
                    PatchHydra._parameter_allow_full_edit,
                    None,
                )
                # get all the overrides
                full_parameters = PatchHydra._current_task.get_parameters(
                ...
                overrides = [
                    ('+' + o) if (o.startswith('+') and not o.startswith('++')) else o
                    for o in overrides
                ]
            else:
        ...
        return PatchHydra._original_hydra_run(
            self,
            config_name,
            task_function,
            overrides,
    ...
    def _patched_task_function(task_function, a_config, *a_args, **a_kwargs):
        ...
        if not running_remotely() or not PatchHydra._allow_omegaconf_edit:
            ...
            # noinspection PyProtectedMember
            omega_yaml = PatchHydra._current_task._get_configuration_text(
