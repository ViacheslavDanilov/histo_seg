...
class WeightsGradientHistHelper(object):
    ...
    def _sample_histograms(_hist_iters, _histogram_granularity):
        ...
        _cur_idx = np.unique(
            np.sort(np.concatenate((cur_idx_below, cur_idx_above)).astype(np.int64)),
        )
        return _cur_idx
    ...
    def add_histogram(
        ...
        step,
        hist_data,
        increase_histogram_update_call_counter=True,
    ):
        ...
        if isinstance(hist_data, dict):
            ...
            isinstance(hist_data, np.ndarray)
            and len(hist_data.shape) == 2
            and np.atleast_2d(hist_data).shape[1] == 3
        ):
...
class EventTrainsWriter(object):
    ...
    def _decode_image(self, img_str, width=None, height=None, color_channels=None):
        ...
        try:
            ...
            logger.warning(
                'Failed decoding debug image [%s, %s, %s]' % (width, height, color_channels),
            )
        ...
        try:
            ...
            if title_prefix and origin_tag[0] == '_':  # add_scalars tag
                origin_tag = origin_tag[
                    1:
                ]  # Remove the first "_" that was added by the main_tag in tensorboard
            else:
                return ''
        except Exception:
            origin_tag = ''
...
class PatchSummaryToEventTransformer(object):
    ...
    def _patch_summary_to_event_transformer():
        if 'tensorflow' in sys.modules:
            try:
                from tensorflow.python.summary.writer.writer import (  # noqa
        ...
        if 'tensorboardX' in sys.modules:
            try:
                ...
                if PatchSummaryToEventTransformer.__original_getattributeX is None:
                    # noinspection PyUnresolvedReferences
                    from tensorboardX.writer import (  # noqa
    ...
    def _patched_getattribute_(self, attr, get_base):
        ...
        if 'event_writer' not in __dict__ or isinstance(
            __dict__['event_writer'],
            (ProxyEventsWriter, EventTrainsWriter),
        ):
            return get_base(self, attr)
        ...
        try:
            ...
            logdir = None
        defaults_dict = (
            __dict__.get('_trains_defaults') or PatchSummaryToEventTransformer.defaults_dict
        )
...
class _ModelAdapter(object):
    ...
    def __init__(self, model, output_model):
        ...
        super(_ModelAdapter, self).__setattr__(
            '_logger',
            LoggerRoot.get_base_logger(TensorflowBinding),
        )
...
class PatchModelCheckPointCallback(object):
    ...
    def _patched_getattribute(self, attr):
        ...
        defaults_dict = (
            __dict__.get('_trains_defaults') or PatchModelCheckPointCallback.defaults_dict
        )
...
class PatchTensorFlowEager(object):
    ...
    def trains_object(self):
        ...
        return PatchTensorFlowEager.__trains_event_writer.get(
            id(self),
            list(PatchTensorFlowEager.__trains_event_writer.values())[0],
        )
    ...
    def _add_image_event_helper(event_writer, img_data_np, tag, step, **kwargs):
        if (
            ...
            for i in range(2, img_data_np.size):
                ...
                image_tag = (
                    str(tag) + '/sample_{}'.format(i - 2) if img_data_np.size > 3 else str(tag)
                )
        ...
        if 'tensorflow' in sys.modules and not PatchKerasModelIO.__patched_tensorflow:
            ...
            if PatchKerasModelIO.__patched_keras:
                PatchKerasModelIO.__patched_tensorflow = [
                    ...
                    keras_saving_legacy
                    if PatchKerasModelIO.__patched_keras[4] != keras_saving_legacy
                    else None,
                    keras_hdf5 if PatchKerasModelIO.__patched_keras[5] != keras_hdf5 else None,
                    keras_saving_v3
                    if PatchKerasModelIO.__patched_keras[6] != keras_saving_v3
                    else None,
                ]
            else:
                PatchKerasModelIO.__patched_tensorflow = [
        ...
        try:
            ...
            if Network is not None:
                ...
                if hasattr(Sequential.from_config, '__func__'):
                    ...
                    Functional.from_config = _patched_call(
                        Functional.from_config,
                        PatchKerasModelIO._from_config,
                    )
                Functional.save = _patched_call(Functional.save, PatchKerasModelIO._save)
                Functional.save_weights = _patched_call(
                    Functional.save_weights,
                    PatchKerasModelIO._save_weights,
                )
                Functional.load_weights = _patched_call(
                    Functional.load_weights,
                    PatchKerasModelIO._load_weights,
                )

            if keras_saving is not None:
                keras_saving.save_model = _patched_call(
                    keras_saving.save_model,
                    PatchKerasModelIO._save_model,
                )
                keras_saving.load_model = _patched_call(
                    keras_saving.load_model,
                    PatchKerasModelIO._load_model,
                )

            if keras_saving_legacy is not None:
                keras_saving_legacy.save_model = _patched_call(
                    keras_saving_legacy.save_model,
                    PatchKerasModelIO._save_model,
                )
                keras_saving_legacy.load_model = _patched_call(
                    keras_saving_legacy.load_model,
                    PatchKerasModelIO._load_model,
                )
            ...
            if keras_hdf5 is not None:
                ...
                if hasattr(keras_hdf5, 'load_model_from_hdf5'):
                    keras_hdf5.load_model_from_hdf5 = _patched_call(
                        keras_hdf5.load_model_from_hdf5,
                        PatchKerasModelIO._load_model,
                    )
                if hasattr(keras_hdf5, 'save_model_to_hdf5'):
                    keras_hdf5.save_model_to_hdf5 = _patched_call(
                        keras_hdf5.save_model_to_hdf5,
                        PatchKerasModelIO._save_model,
                    )

            if keras_saving_v3 is not None:
                keras_saving_v3.save_model = _patched_call(
                    keras_saving_v3.save_model,
                    PatchKerasModelIO._save_model,
                )
        ...
        try:
            ...
            try:
                Checkpoint.restore = _patched_call(
                    Checkpoint.restore,
                    PatchTensorflowModelIO._ckpt_restore,
                )
            except Exception:
                pass
            # noinspection PyBroadException
            try:
                Checkpoint.write = _patched_call(
                    Checkpoint.write,
                    PatchTensorflowModelIO._ckpt_write,
                )
            except Exception:
                pass
            # noinspection PyBroadException
            try:
                Checkpoint._write = _patched_call(
                    Checkpoint._write,
                    PatchTensorflowModelIO._ckpt_write,
                )
            except Exception:
        ...
        try:
            ...
            try:
                checkpoint.TrackableSaver.save = _patched_call(
                    checkpoint.TrackableSaver.save,
                    PatchTensorflow2ModelIO._save,
                )
            except Exception:
                pass
            ...
            try:
                checkpoint.TrackableSaver.restore = _patched_call(
                    checkpoint.TrackableSaver.restore,
                    PatchTensorflow2ModelIO._restore,
                )
            except Exception:
                pass
