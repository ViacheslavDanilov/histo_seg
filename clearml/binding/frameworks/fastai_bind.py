...
from .tensorflow_bind import IsTensorboardInit, WeightsGradientHistHelper
    ...
    def _on_batch_end(original_fn, recorder, *args, **kwargs):
        ...
        try:
            ...
            if (
                ...
                PatchFastaiV1.__gradient_hist_helpers[id(recorder)] = WeightsGradientHistHelper(
                    logger,
                )
            histograms = []
...
class PatchFastaiV2(object):
    ...
    def patch_model_callback():
        ...
        # noinspection PyBroadException
        try:
            fastai.learner.Learner.fit = _patched_call(
                fastai.learner.Learner.fit,
                PatchFastaiV2._insert_callbacks,
            )
        except Exception:
            pass
    ...
    class PatchFastaiCallbacks(__patch_fastai_callbacks_base):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.__gradient_hist_helper = WeightsGradientHistHelper(
                PatchFastaiV2._current_task.get_logger(),
            )
        ...
        def before_step(self):
            ...
            try:
                ...
                gradients = [
                    x.grad.clone().detach().cpu()
                    for x in self.learn.model.parameters()
                    if x.grad is not None
                ]  # noqa
                if len(gradients) == 0:
                    return
