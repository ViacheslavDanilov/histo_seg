...
from ..frameworks import WeightsFileHandler, _Empty, _patched_call, _patched_call_no_recursion_guard
    ...
    def _patch_lightning_io():
        ...
        try:
            ...
            lightning.pytorch.trainer.Trainer.save_checkpoint = _patched_call(
                lightning.pytorch.trainer.Trainer.save_checkpoint,
                PatchPyTorchModelIO._save,
            )  # noqa

            lightning.pytorch.trainer.Trainer.restore = _patched_call(
                lightning.pytorch.trainer.Trainer.restore,
    ...
    def _patch_pytorch_lightning_io():
        ...
        try:
            ...
            pytorch_lightning.trainer.Trainer.save_checkpoint = _patched_call(
                pytorch_lightning.trainer.Trainer.save_checkpoint,
                PatchPyTorchModelIO._save,
            )  # noqa

            pytorch_lightning.trainer.Trainer.restore = _patched_call(
                pytorch_lightning.trainer.Trainer.restore,
                PatchPyTorchModelIO._load_from_obj,
            )  # noqa
        except ImportError:
            pass
        ...
        try:
            ...
            # noinspection PyUnresolvedReferences
            pytorch_lightning.trainer.connectors.checkpoint_connector.CheckpointConnector.save_checkpoint = _patched_call(
                pytorch_lightning.trainer.connectors.checkpoint_connector.CheckpointConnector.save_checkpoint,
                PatchPyTorchModelIO._save,
            )  # noqa

            # noinspection PyUnresolvedReferences
            pytorch_lightning.trainer.connectors.checkpoint_connector.CheckpointConnector.restore = _patched_call(
