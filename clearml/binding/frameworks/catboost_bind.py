...
class PatchCatBoostModelIO(PatchBaseModelIO):
    ...
    def _patch_model_io():
        ...
        try:
            ...
            PatchCatBoostModelIO.__callback_cls = (
                PatchCatBoostModelIO._generate_training_callback_class()
            )
            CatBoost.fit = _patched_call(CatBoost.fit, PatchCatBoostModelIO._fit)
            CatBoostClassifier.fit = _patched_call(
                CatBoostClassifier.fit,
                PatchCatBoostModelIO._fit,
            )
    ...
    def _fit(original_fn, obj, *args, **kwargs):
        ...
        kwargs['callbacks'] = callbacks + [
            PatchCatBoostModelIO.__callback_cls(task=PatchCatBoostModelIO._current_task),
        ]
        # noinspection PyBroadException
        try:
            return original_fn(obj, *args, **kwargs)
